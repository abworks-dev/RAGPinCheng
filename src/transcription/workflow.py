...
def build_pending_job(
    *,
    ...
    return TranscriptionJobRecord(
        ...
        audio_finished_at=audio_finished_at,
    )


def build_pending_preparation_job(
    *,
    job_id: str,
    media_id: str,
    request_idempotency_key: str,
    ...
    """Build a job record whose audio input has not been prepared yet.

    The transcription_jobs row is created immediately (status=pending,
    stage=preparing_audio) so the task list can show the preparation phase
    while ffmpeg extraction runs inside the worker.  The audio fingerprint,
    execution identity and execution config are filled in by
    finalize_prepared_job_input once the worker has prepared the WAV.
    """
    return TranscriptionJobRecord(
        id=job_id,
        media_id=media_id,
        ...
        processed_ms=0,
        status=TranscriptionJobStatus.pending,
        stage=TranscriptionJobStage.preparing_audio,
        failure_error_code=None,
        failure_classification=None,
        error_summary=None,
        checkpoint=None,
