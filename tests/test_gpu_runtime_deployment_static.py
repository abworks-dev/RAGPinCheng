...
def test_runtime_lock_is_validated_by_matching_cuda_qualification():
    ...
    assert metadata["qualification_run_id"] == "31925221286"
    assert metadata["source_commit"] == "1215cc825be45ad3e9dacf0cb909ce4742479ee4"
    assert metadata["qualified_source_fingerprint"] == (
        "e351a5d5d78f554dda43085cd173a47dca94542186ae86abdc83c4e6cb2d9e92"
    )
...
def test_candidate_resolver_is_manual_d_drive_isolated_and_evidence_only():
    ...
    assert "Refusing to stop an unexpected process listening on TCP 8100" in legacy
    ...
    assert '-File "{0}" -ReleaseRoot "{1}" -RepositoryPath "{2}" -RuntimeRoot "{3}" -GpuServiceHost "{4}"' in promotion
    ...
    assert 'Join-Path $resolvedRuntime "releases"' in task_wrapper
    assert "$env:PRODUCTION_REPO_PATH = $resolvedRepository" in task_wrapper
    assert "$env:PRODUCTION_RUNTIME_ROOT = $resolvedRuntime" in task_wrapper
    assert "$env:GPU_SERVICE_HOST = $GpuServiceHost" in task_wrapper
