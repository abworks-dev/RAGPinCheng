...
def test_production_workflow_has_recovery_preview_apply_and_manifest_gates() -> None:
    for contract in (
        ...
        'docker ps -a \\\n',
        'docker restart --timeout 30 "${LIBREOFFICE_ID}"',
...
def test_production_workflow_keeps_repair_scope_narrow() -> None:
    for forbidden in (
        ...
        "docker stop",
        "restart backend",
        "python /app/scripts/repair_pptx_previews.py",
        "git reset",
        "git clean",
        "qdrant",
        "parents.sqlite",
