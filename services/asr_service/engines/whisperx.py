...
from services.asr_service.engine_protocol import EngineChunkCandidate, PreparedAudioChunk, ServiceEngineCapabilities, ServiceProfileConfig
    ...
    def transcribe_chunk(self, chunk: PreparedAudioChunk, config: ServiceProfileConfig) -> EngineChunkCandidate | ProviderFailure:
        ...
        try:
            ...
            seg_iter, _info = model.transcribe(
                ...
                # 也不设置 no_repeat_ngram_size：曾用 3-gram 禁令抑制热词复读，但生产实测它让
                # 解码在重复/不确定语音处提前结束并整段跳过——同一条楼梯视频 690–840 秒区间
                # 覆盖从 99.8% 掉到 69.2%、最长空档 23.0 秒，热词填空段反而更多（2 段 vs 0）。
                # 去掉后同区间 18 段/444 字、覆盖 99.8%、最长空档 0.3 秒；代价是偶发 1 段字符
                # 复读，由应用层 terminology 的退化重复折叠处理，而不是牺牲整段内容。
