...
- 修改前先识别现有设计tokens、共享UI组件和同类页面；优先复用，不为单页复制视觉系统。管理后台同时遵守`project-docs/design/admin-ui-visual-contract.md`（存在时）。
...
- 用户可见变化完成技术验证后，按`project-docs/USER_ACCEPTANCE.md`提供验收交接；未经用户确认不得声称用户验收通过。
