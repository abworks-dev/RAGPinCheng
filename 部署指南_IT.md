...
应用已迁移为双节点架构：
- **Ubuntu 应用节点**（192.168.11.12）：运行 Web 服务 + Qdrant 向量数据库
- **Windows GPU 节点**（192.168.11.11）：运行 GPU 推理服务
...
```bash
curl http://192.168.11.12/api/health
```
...
```bash
curl http://192.168.11.11:8100/health
```
