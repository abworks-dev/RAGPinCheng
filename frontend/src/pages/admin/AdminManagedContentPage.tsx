...
function AdminManagedContentPageContent() {
  ...
  return <section className="space-y-5" aria-labelledby="managed-content-title">
    ...
    <Card className="overflow-hidden shadow-surface [&_table]:!min-w-[56rem]" aria-labelledby="managed-list-title">
      ...
      <div className="border-b border-border bg-surface-muted/40 px-4 py-3 sm:px-5" data-testid="managed-folder-address"><div className="flex items-center rounded-ui-md border border-input bg-background px-3 py-2"><label className="flex w-full items-center gap-2 text-ui-xs font-medium text-muted-foreground"><span className="shrink-0">资料目录</span><Select aria-label="一级目录" className="h-8 min-w-44" value={currentRootFolderId} onChange={(event) => setCurrentFolderId(event.target.value)}><option value="">全部一级目录</option>{rootFolders.map((folder) => <option key={folder.id} value={folder.id}>{folder.display_code} {folder.display_name}</option>)}</Select></label></div></div>
