...
import { Check, Download, Eye, FileText, RefreshCw, Rocket, Search, Send, Upload, X } from "lucide-react";
...
export function AdminManagedContentPage() {
  ...
  return <section className="space-y-5" aria-labelledby="managed-content-title">
    ...
    <section className="grid grid-cols-2 gap-px overflow-hidden border border-border bg-border sm:grid-cols-4" aria-label="资料状态概览">
      {[["全部资料", Object.values(counts).reduce((sum, value) => sum + value, 0)], ["待确认", counts.awaiting_review || 0], ["已确认", counts.approved || 0], ["已发布", counts.published || 0]].map(([label, value]) => <div key={label} className="bg-background px-4 py-3"><p className="text-ui-xs text-muted-foreground">{label}</p><p className="mt-1 text-ui-xl font-semibold">{value}</p></div>)}
    </section>
    ...
    {can("organize") && <section className="space-y-4 border-y border-border py-5" aria-labelledby="managed-upload-title">
      <div><h2 id="managed-upload-title" className="text-ui-base font-semibold">上传资料</h2><p className="mt-1 text-ui-xs text-muted-foreground">上传后先进入待提交状态，不会直接进入检索。</p></div>
      <div className="grid gap-4 lg:grid-cols-[minmax(14rem,18rem)_minmax(0,1fr)_auto] lg:items-end">
        <label className="space-y-1.5 text-ui-sm font-medium"><span>1. 资料分类</span><Select value={uploadCategoryId} onChange={(event) => setUploadCategoryId(event.target.value)} disabled={loading || uploading || categories.length === 0}>{categories.map((category) => <option key={category.id} value={category.id}>{category.full_path || `${category.display_code} ${category.display_name}`}</option>)}</Select></label>
        <div className="space-y-1.5"><span className="block text-ui-sm font-medium">2. 资料文件</span><label className="flex min-h-20 cursor-pointer items-center gap-3 rounded-ui-md border border-dashed border-input bg-background px-4 py-3 hover:bg-surface-muted focus-within:ring-2 focus-within:ring-ring"><FileText className="size-5 shrink-0 text-muted-foreground" /><span className="min-w-0 flex-1"><span className="block text-ui-sm font-medium">{files.length ? `已选择 ${files.length} 个文件` : "选择一个或多个文件"}</span><span className="mt-0.5 block text-ui-xs text-muted-foreground">PDF、Markdown、Word、Excel 或 PPT</span></span><span className="shrink-0 text-ui-sm font-medium text-primary">浏览</span><input ref={fileInputRef} aria-label="选择资料文件" type="file" multiple accept=".pdf,.md,.docx,.xlsx,.pptx" className="sr-only" disabled={uploading} onChange={(event) => { setFiles(Array.from(event.target.files || [])); setUploadResults([]); }} /></label></div>
        <Button className="w-full lg:w-auto" onClick={() => void upload()} disabled={!enabled || uploading || !uploadCategoryId || files.length === 0}><Upload className="size-4" />{uploading ? "上传中…" : "3. 上传"}</Button>
      </div>
      {uploadResults.length > 0 && <ul className="divide-y divide-border border-y border-border text-ui-sm" aria-live="polite">{uploadResults.map((entry) => <li key={entry.filename} className="flex justify-between gap-3 py-2"><span className="break-all">{entry.filename}</span><Badge variant={entry.status === "accepted" ? "success" : "warning"}>{entry.status === "accepted" ? "已接收" : "已跳过"}</Badge></li>)}</ul>}
    </section>}

    <section className="space-y-3" aria-labelledby="managed-list-title">
      <div><h2 id="managed-list-title" className="text-ui-base font-semibold">资料列表</h2><p className="mt-1 text-ui-xs text-muted-foreground">分类以数据库中的编号和名称为准。</p></div>
      <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[minmax(14rem,1fr)_14rem_11rem_11rem_auto] xl:items-end">
        <label className="relative block md:col-span-2 xl:col-span-1"><span className="sr-only">搜索资料</span><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={queryInput} onChange={(event) => setQueryInput(event.target.value)} placeholder="搜索名称、文件名或分类…" /></label>
