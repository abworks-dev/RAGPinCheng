          ...
          <div>
            ...
            {videoFile && (
              <div className="text-xs text-muted mt-1">
                {videoFile.name} · {fmtBytes(videoFile.size)}
              </div>
            )}
          </div>
          <div>
            ...
            {transcriptFile && (
              <div className="text-xs text-muted mt-1">
                {transcriptFile.name} · {fmtBytes(transcriptFile.size)}
              </div>
            )}
          </div>
          <button
            ...
            <table className="w-full text-sm">
              ...
              {mediaAssets.map((m) => (
                <tr key={m.media_id} className="border-t border-gray-100 dark:border-gray-800">
                  ...
                  <td className="px-2 py-1.5 text-muted">{fmtBytes(m.file_size)}</td>
                  ...
                  <td className="px-2 py-1.5 text-muted text-xs">{fmtDate(m.created_at)}</td>
                </tr>
              ))}
              </tbody>
            </table>
