...
const messages: ChatMessage[] = [
  { id: "assistant-1", role: "assistant", content: "较早回答", sources: sources(5) },
  { id: "assistant-2", role: "assistant", content: "最新回答", sources: sources(7) },
];

describe("getSelectedSourceCount", () => {
  it("uses the source count of the currently selected answer", () => {
    expect(getSelectedSourceCount(messages, "assistant-1")).toBe(5);
  });

  it("falls back to the latest sourced answer without a valid selection", () => {
    expect(getSelectedSourceCount(messages, null)).toBe(7);
    expect(getSelectedSourceCount(messages, "missing")).toBe(7);
  });
});
