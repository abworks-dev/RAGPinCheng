...
export function CategoryTreePicker({
  ...
  const focusableCategoryId = visibleNodes.some((node) => node.category.id === value)
    ? value
...
function CategoryTreePickerNode({
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === "Home" && visibleNodes[0]) { event.preventDefault(); onFocusNode(visibleNodes[0].category.id); }
    ...
    if ((event.key === "Enter" || event.key === " ") && !isCurrent && !disabled) {
      event.preventDefault();
  ...
  return <>
    <div
      ...
      onClick={() => { if (!isCurrent && !disabled) onSelect(category.id); }}
      onKeyDown={onKeyDown}
      className={`relative flex min-h-14 items-start gap-2 border-l-2 py-3 pr-3 outline-none transition-colors duration-normal focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring ${level === 1 ? "pl-3" : "pl-2 before:absolute before:left-0 before:top-7 before:w-2 before:border-t before:border-border/70"} ${isSelected ? "border-l-primary bg-primary/10" : isCurrent ? "border-l-transparent bg-surface-muted/50" : "border-l-transparent hover:bg-surface-muted/60"} ${isCurrent || disabled ? "cursor-not-allowed opacity-70" : "cursor-pointer"}`}
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-1 block break-words text-ui-xs text-muted-foreground">{category.item_count} 份直接资料{hasChildren ? ` · ${children.length} 个子目录` : ""}</span>
      </span>
    </div>
    {hasChildren && isExpanded && <div role="group" className="ml-5 border-l border-border/70 bg-surface-muted/10 sm:ml-6">{children.map((child, childIndex) => <CategoryTreePickerNode key={child.category.id} node={child} level={level + 1} index={childIndex} siblingCount={children.length} selectedId={selectedId} focusableCategoryId={focusableCategoryId} currentCategoryId={currentCategoryId} disabled={disabled} expanded={expanded} visibleNodes={visibleNodes} nodeRefs={nodeRefs} onSelect={onSelect} onToggle={onToggle} onFocusNode={onFocusNode} />)}</div>}
  </>;
}
