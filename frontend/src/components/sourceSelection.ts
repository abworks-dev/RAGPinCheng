...
export function selectedSourceSet(messages: ChatMessage[], selectedMessageId: string | null): SourceSet | undefined {
  const sets = sourceSetsFromMessages(messages);
  const latest = sets[sets.length - 1];
  return sets.find((set) => set.messageId === selectedMessageId) || latest;
}

export function getSelectedSourceCount(messages: ChatMessage[], selectedMessageId: string | null): number {
