...
```bash
curl -s http://${PRIVATE_IPV4}:8100/health
# 返回: {"status":"ok","model_loaded":true}

curl -s http://${PRIVATE_IPV4}:8100/model-info
...
```bash
# SQLite 备份
cp ${PRODUCTION_APP_DATA_PATH}/app.sqlite ${PRODUCTION_APP_BACKUP_DIRECTORY}/app-$(date +%Y%m%d).sqlite
cp ${PRODUCTION_APP_DATA_PATH}/parents.sqlite ${PRODUCTION_APP_BACKUP_DIRECTORY}/parents-$(date +%Y%m%d).sqlite
...
访问 `http://${PRIVATE_IPV4}`（Ubuntu 内网地址）：
