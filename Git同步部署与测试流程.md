...
- 服务器代理：`http://10.205.165.230:7897`
- Qdrant 固定版本：`v1.18.3`
...
服务器需要安装 runner，并让其服务账号具备访问 `D:\RAGPinCheng`、`D:\RAGBackups`、Git 和 Docker 的权限。不要把 `.env`、API Key、管理员密码或生产数据放入 GitHub Actions。

如果构建需要代理，在 GitHub 仓库的 **Settings → Secrets and variables → Actions → Variables** 中配置仓库级非敏感变量：

```text
DEPLOY_HTTP_PROXY=http://10.205.165.230:7897
```
...
$oldVersion | Set-Content D:\RAGBackups\last-production-commit.txt
```
...
```powershell
git config --local http.proxy http://10.205.165.230:7897
```
...
```powershell
$proxy = "http://10.205.165.230:7897"
...
```powershell
$oldVersion = Get-Content D:\RAGBackups\last-production-commit.txt
