...
- Windows 生产目录：`D:\RAGPinCheng`
- Ubuntu 生产目录：`/data/workspace/projects/ragpincheng`
...
| 日志 | `D:\RAGPinCheng\gpu_service.log` |
...
| 内网 IP | 192.168.11.12 |
| 系统 | Ubuntu 24.04 LTS |
| 仓库路径 | `/data/workspace/projects/ragpincheng` |
| 数据路径 | `/data/services/docker/data/ragpincheng/prod/app/` |
...
| Compose 覆盖 | `/data/services/docker/compose/ragpincheng/prod/compose.prod.yaml` |
| 环境变量 | `/data/secrets/ragpincheng/prod.env` |
...
```bash
cd /data/workspace/projects/ragpincheng
git pull
...
# Ubuntu backend 日志
sudo docker compose -p ragpincheng-prod \
  -f /data/workspace/projects/ragpincheng/docker/docker-compose.yml \
  -f /data/services/docker/compose/ragpincheng/prod/compose.prod.yaml \
  --env-file /data/secrets/ragpincheng/prod.env \
  logs -f backend
...
```bash
curl -s http://192.168.11.11:8100/health
# 返回: {"status":"ok","model_loaded":true}

curl -s http://192.168.11.11:8100/model-info
...
```bash
# SQLite 备份
cp /data/services/docker/data/ragpincheng/prod/app/app.sqlite /data/backup/databases/ragpincheng/app-$(date +%Y%m%d).sqlite
cp /data/services/docker/data/ragpincheng/prod/app/parents.sqlite /data/backup/databases/ragpincheng/parents-$(date +%Y%m%d).sqlite
...
访问 `http://192.168.11.12`（Ubuntu 内网地址）：
...
docs/
media/
/data/secrets/ragpincheng/prod.env（Ubuntu）
...
sudo rm -rf /data/services/docker/data/ragpincheng/prod/qdrant
```
