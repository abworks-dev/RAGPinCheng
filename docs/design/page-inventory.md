...
| 资料管理 | `/admin/content` | 通过资料库、回收站、上传任务、索引任务和转录任务页签完成统一上传、确认、发布、追踪与恢复 | 各页签按 `item.view`、`trash.view`、`item.upload`、`index.view` 显示；转录任务和 MP4 上传首期仅系统管理员 | 受管 content、category、permission、upload task、index job、media/transcription API | L/E/X/D/B/S/P | [managed-content-library](../features/managed-content-library.md)、[transcript-pipeline](../features/transcript-pipeline.md)、[admin visual contract](admin-ui-visual-contract.md) | `AdminManagedContentPage.test.tsx` | `admin-workflows.spec.ts`、`admin-golden.spec.ts`、`media-page.spec.ts` | [manual-regression](../../frontend/tests/manual-regression.md) |
...
| 索引任务页签 | `/admin/content?view=index` | 受管资料发布任务状态、筛选、历史尝试、文件查看和失败重试；旧 `/admin/index` 重定向至此 | `index.view`；重新发布另需 `item.publish` | 受管 index job API | L/E/X/D/B/S/P | [document-indexing](../features/document-indexing.md)、[admin visual contract](admin-ui-visual-contract.md) | `AdminDocumentsPage.test.tsx`、`AdminManagedContentPage.test.tsx` | `admin-workflows.spec.ts`、`admin-golden.spec.ts` | [manual-regression](../../frontend/tests/manual-regression.md) |
| 转录任务页签 | `/admin/content?view=transcription` | 查看视频独立阶段、取消/重试转录、校对审核、发布、替换、归档和失败记录处理；旧 `/admin/media` 保留查询参数后重定向至此 | 系统管理员；转录方案仍受运行时准入策略限制 | media、transcription job/scheme、version、publication API | L/E/X/D/B/S/P | [transcript-pipeline](../features/transcript-pipeline.md)、[admin visual contract](admin-ui-visual-contract.md) | `AdminMediaPage.test.tsx`、`AdminManagedContentPage.test.tsx`、`useAdminMediaAssets.test.ts`、`useTranscriptionJobs.test.ts` | `media-page.spec.ts` | [manual-regression](../../frontend/tests/manual-regression.md) |
| 转录配置 | `/admin/asr` | 比较服务器固定 Profile、分段规则、运行身份和发布申请审计 | 管理员 | `/api/admin/asr`、`/api/admin/asr/release-requests` | L/E/X/D/B/S | [transcript-pipeline](../features/transcript-pipeline.md)、[admin visual contract](admin-ui-visual-contract.md) | `AdminAsrSettingsPage.test.tsx` | `asr-settings.spec.ts` | [USER_ACCEPTANCE](../USER_ACCEPTANCE.md)、[manual-regression](../../frontend/tests/manual-regression.md) |
