...
| 资料管理 | `/admin/content` | 通过资料库、回收站、上传任务和索引任务页签完成上传、确认、发布、追踪与恢复 | 各页签按 `item.view`、`trash.view`、`item.upload`、`index.view` 显示；管理员全量 | 受管 content、category、permission、upload task、index job API | L/E/X/D/B/S/P | [document-indexing](../features/document-indexing.md)、[admin visual contract](admin-ui-visual-contract.md) | `AdminManagedContentPage.test.tsx` | `admin-workflows.spec.ts`、`admin-golden.spec.ts` | [manual-regression](../../frontend/tests/manual-regression.md) |
