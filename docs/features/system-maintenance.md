...
管理员可在独立的“系统维护”页面查看自动清理状态，配置历史对话保留期限和受管资料上传限制，预览待清理数量，手动执行清理并查看最近运行记录。默认策略为启用自动清理并保留 30 天；资料上传默认限制为单文件 2000 MB、单批 5000 个文件、单批总量 10240 MB。管理概览另外提供应用节点与 GPU 节点的当前资源摘要。
...
- 资料上传限制持久化保存，允许配置单文件 1 至 10240 MB、单批 1 至 10000 个文件、单批总量 1 至 102400 MB，且单批总量不得小于单文件上限；保存后新上传请求立即读取最新值；
...
→ conversations / messages / auth_sessions / managed-content upload routes
```
...
- `upload_max_file_mb`、`upload_max_batch_files` 和 `upload_max_batch_mb` 分别控制单文件、单批数量和单批总量；预检、正式上传、资料更新和 capabilities 使用同一份设置；
...
- 资料权限用户只能从 capabilities 读取当前生效限制，只有管理员可以修改限制；
- 设置保存和预览不得执行删除；
- 登录会话期限与对话保留策略独立；
...
- 验证默认策略、上传限制、设置边界、预览只读、自动清理开关、手动清理和外键级联；
