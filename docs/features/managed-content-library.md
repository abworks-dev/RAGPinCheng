...
已实现。普通文档经过上传、提交、确认和发布后进入检索；已正式发布的视频转录稿也会以视频条目出现在受管目录中。资料管理页按当前受控目录展示两类资料，支持上传、拖放确认、新建目录、搜索、状态/来源/类型筛选、批量流程操作，以及与资料类型匹配的单项操作。回收站后新增“上传任务”页，集中展示当前账号的上传历史和当前浏览器内的传输进度。
...
- 普通文档提供查看、预览、移动、下载、重命名、更新和删除操作；提交、确认、退回和发布保留在详情弹窗中。视频转录稿只提供详情、播放、移动目录和进入视频管理，校对、发布、改名和完整删除仍由视频管理负责。
