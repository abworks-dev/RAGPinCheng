...
共享媒体目录在分类树中以“共享文件夹”显示。共享文件夹只读扫描 MP4，远程视频不可通过资料库移动、重命名、替换或删除，但其转录稿继续使用既有发布决策、发布和索引流程。

资料列表进入共享文件夹后继续使用原有资料条目结构：共享子目录沿用文件夹行和逐级进入交互，共享视频沿用视频资料行，额外显示“共享”、来源和只读状态。共享条目替换普通资料分页区域，页面不会同时平铺全部外部视频和显示第二套目录浏览器；每一层只显示直属子文件夹与直属视频。

分类管理在共享分类根节点下展示远程子目录的受管镜像节点。共享子目录与普通子分类使用同一条目样式（编号、启用状态、问答/筛选徽标和条目统计），可独立设置启用状态、纳入企业知识问答与对话筛选项，也可调整编号（仅影响本系统内显示顺序，不影响远程目录和媒体归属）；但名称跟随远程目录，不能移动、改名或删除，也不能在其下新建或移入其他分类。镜像由共享目录扫描（手动或周期）按远程目录增删维护：新目录追加到同级末尾，消失的远程目录软删除并重排同级编号；既有镜像的设置与编号不会被后续扫描改写。共享文件不进入分类树，仍由资料库的远程目录浏览展示。

已实现。普通文档上传后进入“待发布”，发布完成后进入检索；系统管理员上传的 MP4 从同一资料上传入口进入资料库，初始状态为“待转录”，不会在上传时创建转录任务。管理员可在资料列表选择视频，或在“上传任务”按上传批次（含分散在子目录中的视频）选择转录方案后，视频才进入独立转录链路；正式发布后的转录稿继续绑定同一视频条目。“转录任务”作为资料管理的管理员子页签承载视频后续处理。
...
- 影响范围按目录与完整路径展示。状态或权限不匹配的资料保留在树中并标明原因，不会被流程操作修改；可操作资料可逐项取消。
...
- 文件冲突默认选择跳过，也可另存为新资料（默认建议追加 ` (1)` 等后缀），或在已有资料没有活动发布/整理任务时作为其新待发布版本。文件夹根目录冲突默认不合并，支持合并到现有目录或整体重命名后重新预检；重命名不会改变文件夹内的相对层级。
...
-> pending_publication -> content_index_jobs -> content_item_heads
...
-> pending_publication -> 发布任务 -> content_item_heads
...
只读外部媒体源通过受管镜像节点展示远程目录层级。共享视频完成现有发布决策、索引和发布事务后才作为 `media_transcript` 出现在本资料库；其转录索引额外携带所属镜像目录的 `category_key`（归属算法见 [外部媒体源](external-media-sources.md)），因此可参与共享子目录级别的问答范围与对话筛选。源扫描、缺失和不可达状态见 [外部媒体源](external-media-sources.md)。
...
Schema 19 增加 `media_metadata_revisions` 和 `media_replacements`。媒体标题/源文件名修订复用当前正式 Markdown 创建待发布候选；替换视频作为新的媒体、转录和索引候选处理。两类候选都在发布决策、索引和发布成功前保留旧 `media_transcript_heads`、目录壳和检索可见内容；最终激活在一个 SQLite 事务内切换 head 与目录关联，失败整笔回滚。旧视频只在替换成功后标记归档，物理文件不在该事务中删除。
...
- 文件夹行显示直接资料、直接子文件夹、递归子文件夹和递归资料统计。选中文件夹后，批量移动、发布和下载递归作用于其子树；删除只接受文件夹根，普通删除沿用空目录约束，强制删除要求独立确认。
...
- PPT/PPTX/XLS/XLSX 发布时通过 LibreOffice 生成同版本 `.preview.pdf` 派生产物。列表接口以预览文件的 PDF 签名为准返回 `preview_status`，缺失或损坏时不再开放预览按钮；发布负责人可在资料详情中单独重新生成预览，该操作不重新索引，也不改变资料发布状态。转换写入使用同目录临时文件和原子替换，失败不会破坏已有有效预览。XLS/XLSX 均优先读取同版本 `.preview.xlsx` 派生产物，避免旧格式与新格式走不同预览读取路径。
...
- XMind 预览使用版本级 `item.view` 接口读取受管对象，返回受限的画布与主题树；资料列表和详情入口均复用 `ResourcePreviewShell`，与 PDF、Office 和视频预览保持同一抽屉动效、焦点恢复和移动端全屏行为。主题树由本地只读思维导图画布渲染为中心主题、双向分支和连线，支持画布切换、平移、缩放与适配视图；原始资料不会发送到第三方渲染服务。预览不依赖发布后的 `parent_id`，因此待发布和已发布版本均可查看。
- PPTX 预览生成失败仍不阻断资料索引与发布。PDF 预览窗口显示中文失败原因；系统概览实际探测 LibreOffice `/health`，区分“运行正常”“服务异常”和“已停用”。
- 视频上传后在资料列表显示“待发布”，行主操作为“发布”；选中视频后的操作同样为“批量发布”，资料页不再提供单个或批量转录。首次发布创建视频发布意图，不创建实际索引任务，并把视频同时加入发布任务和转录任务。发布任务列表中处于“待转录/转录稿就绪待发布”阶段的视频条目可“取消发布”，取消发布只允许在意图进入正式发布（`publishing`）之前，取消后资料列表恢复“待发布”，进行中的转录任务一并取消、媒体重置为可重新转录，并写入 `content.publication_cancelled` 审计事件；同一事务内完成，之后可重新发布。转录方案、批量选择、逐视频方案覆盖、发布决策及最终发布都在转录任务页完成；只有最终发布开始后资料列表才显示“发布中”，索引成功并自动切换正式 head 后显示“已发布”。播放、下载在正式 head 发布前保持禁用。
- 转录任务页每条记录在处理进度后展示所用转录方案（仅已选择方案进入转录或转录完成的记录），服务端解析方案名并在自定义方案被归档（或纪录缺失）时标记“原转录配置已删除”；转写工作台头部与各转录版本卡片同样展示。行操作“转录/重新转录/工作台/调整目录/取消/重试/退回待发布/替换视频/清理/移入回收站”全部常显，不满足条件时禁用并由 `disabled_actions` 提供悬浮原因；“重新转录”（含批量）弹出方案选择对话框，所选方案与原方案一致时给出提示但仍可继续，新的转录成功只生成待发布新版本，正式 head 保留到新稿作出发布决策并发布。
...
- 编辑媒体信息不会立即修改正式条目。候选稿必须重新作出发布决策、索引并发布，成功后标题、源文件名、目录壳和正式 head 同时生效；发布决策拒绝或索引/事务失败时旧名称和旧 head 保持不变。
...
- 重命名同时修改资料标题和源文件名；更新上传新对象，并可沿用原名称或使用上传文件名。两者都会新增递增的待发布版本，不覆盖历史版本或对象。
- 已发布资料生成新待发布版本时，旧 `content_item_heads` 保持有效，直到新版本发布成功，避免检索空窗。此期间不能移动或由仅有整理权限的账号删除该资料。
...
- `POST /api/admin/content/items/{item_id}/publish`（视频首次发布，只创建发布意图）
- `POST /api/admin/content/publication-jobs/{job_id}/cancel`（取消处于待转录/转录稿就绪待发布阶段的视频发布意图；要求管理员 + CSRF）
...
- 资料整理：`item.upload`、`item.move_draft`、`item.archive_draft` 分别控制上传、移动待发布资料和将其移入回收站。后两个权限键暂时保留历史命名，当前实际作用对象均为 `pending_publication`。
- 发布：普通文档上传和更新后直接进入 `pending_publication`（待发布）；`item.publish` 控制发布和重新生成当前已发布 PPT/PPTX/XLS/XLSX 的预览，`item.archive_published` 控制将发布失败或已发布资料移入回收站。旧的普通文档提交、确认和退回接口已移除；视频转录稿发布决策流程保持独立。
...
`GET /api/admin/content/trash` 要求 `trash.view`；`POST /api/admin/content/items/{item_id}/restore` 额外要求 `trash.restore`、当前版本号和 CSRF token。普通文档统一恢复为 `pending_publication`（待发布）。恢复不会重建 `content_item_heads`，不会自动进入检索，必须重新发布。
...
系统管理员可对 1–20 份已选普通文档或视频执行永久删除。`POST /api/admin/content/trash/purge/preflight` 检查归档状态、当前版本、活动索引/分类调整任务，以及视频的活动转录、发布、待发布修订、媒体信息修订、替换任务和所有文件路径边界；`POST /api/admin/content/trash/purge` 再次执行相同检查。普通资料要求输入精确的“永久删除 N 份资料”，含视频的批次要求输入服务端返回的“永久删除 N 份资料（含 M 个视频）”。
