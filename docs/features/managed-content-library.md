...
- 视频上传后在资料列表显示“待发布”，行主操作为“发布”；选中视频后的操作同样为“批量发布”，资料页不再提供单个或批量转录。首次发布创建视频发布意图，不创建实际索引任务，并把视频同时加入发布任务和转录任务。转录方案、批量选择、逐视频方案覆盖、审核修改及最终发布都在转录任务页完成；只有最终发布开始后资料列表才显示“发布中”，索引成功并自动切换正式 head 后显示“已发布”。播放、下载在正式 head 发布前保持禁用。
