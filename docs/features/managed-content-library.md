...
只读外部媒体源通过受管镜像节点展示远程目录层级。共享视频完成现有审核、索引和发布事务后才作为 `media_transcript` 出现在本资料库；其转录索引额外携带所属镜像目录的 `category_key`（归属算法见 [外部媒体源](external-media-sources.md)），因此可参与共享子目录级别的问答范围与对话筛选。源扫描、缺失和不可达状态见 [外部媒体源](external-media-sources.md)。
...
- 视频上传后在资料列表显示“待发布”，行主操作为“发布”；选中视频后的操作同样为“批量发布”，资料页不再提供单个或批量转录。首次发布创建视频发布意图，不创建实际索引任务，并把视频同时加入发布任务和转录任务。发布任务列表中处于“待转录/转录稿就绪待发布”阶段的视频条目可“取消发布”，取消发布只允许在意图进入正式发布（`publishing`）之前，取消后资料列表恢复“待发布”，进行中的转录任务一并取消、媒体重置为可重新转录，并写入 `content.publication_cancelled` 审计事件；同一事务内完成，之后可重新发布。转录方案、批量选择、逐视频方案覆盖、审核修改及最终发布都在转录任务页完成；只有最终发布开始后资料列表才显示“发布中”，索引成功并自动切换正式 head 后显示“已发布”。播放、下载在正式 head 发布前保持禁用。
- 转录任务页每条记录在处理进度后展示所用转录方案（仅已选择方案进入转录或转录完成的记录），服务端解析方案名并在自定义方案被归档（或纪录缺失）时标记“原转录配置已删除”；转写工作台头部与各转录版本卡片同样展示。行操作“转录/重新转录/工作台/调整目录/取消/重试/退回审核/替换视频/清理/移入回收站”全部常显，不满足条件时禁用并由 `disabled_actions` 提供悬浮原因；“重新转录”（含批量）弹出方案选择对话框，所选方案与原方案一致时给出提示但仍可继续，新的转录成功只生成待审核新版本，正式 head 保留到新稿发布。
...
- 发布：普通文档上传和更新后直接进入 `pending_publication`（待发布）；`item.publish` 控制发布和重新生成当前已发布 PPT/PPTX/XLS/XLSX 的预览，`item.archive_published` 控制将发布失败或已发布资料移入回收站。旧的普通文档提交、确认和退回接口已移除；视频转录稿审核流程保持独立。
