...
受管资料库功能开启后，“资料库”是普通资料唯一的网页上传、确认和发布入口，支持 PDF、Markdown、DOCX、XLSX 和 PPTX。具有独立资料权限的人员按“整理上传 → 负责人确认 → 管理员发布”处理普通资料；分类、业务资料、版本、审核、发布和正式 head 由 `app.sqlite` 管理，只有发布成功的当前版本进入正式检索。“索引监控”只展示受管资料发布任务，不再暴露旧目录索引列表或其重试、重新索引和删除操作。教学视频转录稿仍走既有媒体链路。
...
- “索引监控”以受管资料发布任务为唯一主体，提供状态汇总、名称搜索、数据库分类/文件类型/状态筛选、分页和历史尝试切换；失败任务展示稳定中文原因和建议操作，重试仍从“资料库”发起；
