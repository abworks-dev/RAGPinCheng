...
- “资料库”中的文件夹排在文件资料之前，分类管理员可直接设置排序序号、移动位置、重命名或打开文件夹；移动选择器复用分类目录树并明确禁用自身、后代、同级重名/显示编号冲突和超深目标。显示编号用于地址栏识别且同级唯一，排序序号只决定同级顺序、允许重复，`0` 表示未设置并排在已设置序号之后；同序号按名称和稳定 ID 排序；
