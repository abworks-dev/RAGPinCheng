...
- “资料库”中的文件夹排在文件资料之前，编号继续作为文件夹名称前缀显示，不新增独立“顺序”列；分类管理员可直接调整编号、移动位置、重命名或打开文件夹。编号是唯一的用户侧顺序概念：选择已占用编号时先确认，继续后目标文件夹插入该位置并自动连续重排同级编号；移动到其他目录后排在目标末尾，同时连续重排源目录和目标目录。移动选择器复用分类目录树并明确禁用自身、后代、同级重名和超深目标，不再因旧显示编号碰撞阻止移动；
...
- 分类稳定标识由服务端生成且不在业务界面展示；“分类设置”以可搜索、可按状态筛选的树形主从工作台展示完整层级和直接资料数，桌面端在右侧维护选中分类，平板和移动端使用 Sheet 完成同一编辑流程；详情只读展示当前编号，通过“调整编号”进入与资料库一致的占用确认和连续重排流程，新建分类可选择插入编号；不提供独立排序序号、上移、下移或拖动排序。跨父级移动选择目标父分类后自动追加到末尾并重新编号；含直接资料或启用子分类的分类不得停用；每个活动分类可分别设置是否纳入企业知识问答、是否显示为对话筛选项，后者依赖前者，停用分类会同时关闭两项；
...
- `category_nodes.chat_search_enabled` 控制目录内容是否进入企业知识问答，`chat_filter_selectable` 控制目录是否出现在登录用户的范围筛选器；迁移时活动分类默认同时开启，后续由分类管理员逐项调整；
