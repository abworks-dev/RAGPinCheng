...
function Invoke-SelfTest {
  ...
  $gateGui=Get-AsrProcessGate @() @($gui) '${QUALIFICATION_SANDBOX_ROOT}\faster-whisper-runs\phase0-fw-r3a-retry-test' 999
  Check (-not $gateGui.blocked) 'WDDM GUI evidence incorrectly blocked A1'
  $gateActive=Get-AsrProcessGate @('${QUALIFICATION_SANDBOX_ROOT}\active-run.json') @($gui) '${QUALIFICATION_SANDBOX_ROOT}\faster-whisper-runs\phase0-fw-r3a-retry-test' 999
  Check $gateActive.blocked 'active-run did not block A1'
  $asr=[pscustomobject]@{ProcessId=101;Name='python.exe';ExecutablePath='C:\Python310\python.exe';CommandLine='python.exe ${QUALIFICATION_SANDBOX_ROOT}\faster-whisper-runs\phase0-fw-r3a-retry-test\smoke.py --model faster-whisper'}
  Check ((Get-AsrProcessGate @() @($asr) '${QUALIFICATION_SANDBOX_ROOT}\faster-whisper-runs\phase0-fw-r3a-retry-test' 999).blocked) 'run-bound ASR process did not block A1'
