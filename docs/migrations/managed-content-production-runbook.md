...
- “索引任务”：只查看资料库发布任务，可按名称、数据库分类、文件类型、来源、状态和历史尝试筛选；可打开原文件，并对最新失败尝试重新发布；不提供旧目录索引列表或新资料上传。
...
发布失败时可在“资料库”或“索引任务”查看最新失败原因、可重试性和建议操作；“索引任务”默认每个版本只显示最新尝试，通过“查看历史尝试”核对完整审计记录，并只允许最新失败尝试重新发布。需要密码的 PDF 应先解除密码保护后重新上传；解析服务或索引服务类可重试错误应在依赖恢复后再发布，避免连续重复提交。
...
受控 workflow 为 `.github/workflows/retire-managed-content-legacy-index.yml`，固定确认词 `RETIRE_LEGACY_INDEX_T11`、完整生产 commit、117 条迁移记录（116 个正式 head + 1 个无正式 head、无活动任务且未发布的生成预览）和 `compat` 模式。执行前必须满足四类活动任务均为零，并生成冻结的 Parent ID/Point ID 清单。清单只保存在独立备份目录，公开 artifact 仅包含聚合计数。
...
`legacy-docs` 宿主机兼容入口废弃后，最终 overlay 使用 `!override` 完整替换 backend volumes，显式保留生产 `/app/data`、`/app/content` 和 `/app/media`，并使用空的只读 tmpfs 覆盖 `/app/docs`，不再读取 `DOCS_HOST_PATH`。生产成功标准为 `/app/docs` 的 mount type 是 `tmpfs`、容器 mountinfo 包含 `ro` 且实际写入探针失败，`/app/media → content/media`、`/app/content → content`，同时所有宿主机 mount source 均不得位于 `/data/business/ragpincheng/source`。`.github/workflows/retire-production-legacy-docs-directory.yml` 只在部署该契约后处理精确路径 `/data/business/ragpincheng/content/legacy-docs`：要求目录为空、不是符号链接、不是任何 mount source，保存 uid/gid/mode 元数据后仅执行 `rmdir`；失败时重建同权限空目录。它不处理 `source/docs`、`source/media`、对象存储、数据库或索引。
