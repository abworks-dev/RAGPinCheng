...
- 验证：激活、既有部署和模型缓存专项测试 `42 passed`；本地可运行的 ASR/Provider 回归 `146 passed`；Python `py_compile`、PowerShell AST 解析、WORKLOG 标题格式和 `git diff --check` 通过；PR #19～#28 各七个 CI job 全部通过。生产 preflight 已依次验证并修复 Windows checkout、受控 GPU probe Token、防火墙规则归属判断、PowerShell 原生 stderr、capabilities 冷启动、孤儿子进程回滚和固定 GPU activity URL；PR #26 的生产运行证明仅移除 Actions 跟踪标识仍不能让长期 GPU 子进程跨 job 存活，Windows job 本机 smoke 通过后 `8100` 再次停止，Ubuntu 因此拒绝继续部署。现已将 GPU service 改为固定 `RAGPinCheng-GPU` Administrator/S4U Scheduled Task，由严格环境启动脚本以前台子进程承载服务并持久化日志；部署只管理精确任务定义和精确 `Python310 -m gpu_service.app` 监听进程，禁止缺失 Token 时自动生成或轮换，失败自动移除本次任务和监听。首次生产运行在任务注册前的空监听集合触发 StrictMode 属性访问错误，未创建任务，现已将 GPU 与 ASR 的监听 PID 提取统一改为可安全处理空集合的管道形式；第二次生产运行成功启动任务，但 S4U 任务不继承 runner 代理，Hugging Face 对本地 BGE 缓存执行远程 HEAD 并在健康时限内超时，任务随后自动回滚。现已固定并校验 `HF_HUB_OFFLINE=1`、`TRANSFORMERS_OFFLINE=1`，使生产 GPU 只读取既有本地缓存且不下载模型。相关部署专项测试 `39 passed, 6 subtests passed`，生产 GPU 恢复和 ASR 双节点验收待该修复 CI/合并后执行。
