...
- 文件：`.github/workflows/activate-asr-production.yml`、`.github/workflows/ci.yml`、`scripts/activate-asr-production.ps1`、`scripts/verify-asr-service.ps1`、`scripts/verify_asr_from_ubuntu.py`、`scripts/deploy-gpu.ps1`、`tests/test_asr_activation.py`、`tests/test_deploy_git_safety.py`、`project-docs/plans/multi-engine-transcription-phase5c-windows-asr-deployment.md`、`WORKLOG.md`。
- 验证：激活、既有部署和模型缓存专项测试 `42 passed`；本地可运行的 ASR/Provider 回归 `146 passed`；Python `py_compile`、PowerShell AST 解析、WORKLOG 标题格式和 `git diff --check` 通过；PR #19～#25 各七个 CI job 全部通过。生产 preflight 已依次验证并修复 Windows checkout、受控 GPU probe Token、防火墙规则归属判断、PowerShell 原生 stderr、capabilities 冷启动、孤儿子进程回滚和固定 GPU activity URL；管理员 runner 重试进一步确认现有 GPU 部署脚本启动的长期进程继承 Actions 跟踪标识且输出管道未落盘，job 结束后 `8100` 无监听、无进程和日志。现已改为启动时仅对子进程移除 runner 跟踪标识和 checkout Token、分别持久化 stdout/stderr，并在健康失败时输出日志和终止本次精确进程；相关部署专项测试 `38 passed, 6 subtests passed`，生产恢复和 ASR 双节点验收待该修复 CI/合并后执行。
