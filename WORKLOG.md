...
- 验证：workflow YAML、严格 Manifest 单段/空段数组序列化及 `git diff --check` 通过；R3 专项 47/47 通过；本地无外部服务的 ASR/Profile/Remote Provider/部署回归 322/322 通过；PR #36 与编码修复 PR #37 的 7 项 CI 均全部通过，分别以 `45a4ebe6d97847348e6add9c33349d12dadc4fde`、`41de4fefb5dac1649c0590bb4ed75030064151b2` 合并。生产 workflow `30954369811` 暴露并关闭了 Windows PowerShell 5.1 无 BOM UTF-8 中文解析问题；修复后的 `30954705433` 已成功生成和严格校验固定 8 样本，随后因 pip requirements 文件把 checkout 反斜杠当转义符而在依赖下载前失败。路径修复仅将已校验 checkout 路径转换为 `/`，Windows PowerShell 5.1 AST、47 项专项测试和 `git diff --check` 通过。
- 待办/风险：pip 路径修复尚待独立 PR、远端 CI、合并和生产 workflow 重跑；样本已位于固定目录并可按严格 Schema/语义幂等复用。未启动或修改 ASR/GPU 生产服务、Scheduled Task、防火墙、Ubuntu、数据库、Qdrant、业务媒体或 Profile admission。
