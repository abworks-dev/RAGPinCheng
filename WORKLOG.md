...
- 待办/风险：需先由用户从官方索引下载 `torch-2.7.0+cu128-cp310-cp310-win_amd64.whl`，生成manifest并将两者传入Helios固定目录，之后才能重跑候选解析、人工复核锁并以 `candidate` 提交，再执行CUDA/S4U资格验证。自动生产部署保持禁用，不进行promotion，不恢复生产GPU服务，不修改生产任务、全局依赖、模型缓存、数据库、Qdrant或防火墙；已取消run留下的部分resolver缓存不清理。
