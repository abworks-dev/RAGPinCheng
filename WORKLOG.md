...
- 验证：激活、既有部署和模型缓存专项测试 `42 passed`；本地可运行的 ASR/Provider 回归 `146 passed`；Python `py_compile`、两份 PowerShell AST 解析、WORKLOG 标题格式和 `git diff --check` 通过；PR #19～#21 各七个 CI job 全部通过。生产 preflight 已依次验证并修复 Windows checkout 代理和受控 GPU probe Token 注入；第三次在改动生产状态前发现防火墙扫描把其他程序/特定服务规则误判为 ASR 暴露面。现已保持 fail-closed，但只把适用于任意程序/服务或 ASR venv Python、且覆盖 8200 的规则视为冲突，不删除或修改现有规则；该修复的 CI 与生产重跑继续由同一交付完成。
