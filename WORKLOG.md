...
- 验证：激活、既有部署和模型缓存专项测试 `42 passed`；本地可运行的 ASR/Provider 回归 `146 passed`；Python `py_compile`、PowerShell AST 解析、WORKLOG 标题格式和 `git diff --check` 通过；PR #19～#24 各七个 CI job 全部通过。生产 preflight 已依次验证并修复 Windows checkout、受控 GPU probe Token、防火墙规则归属判断、PowerShell 原生 stderr、capabilities 冷启动和孤儿子进程回滚；最新管理员 runner 执行确认完整自动回滚已可终止精确匹配的 Uvicorn，失败点收敛为 GPU service 实际只监听 `192.168.11.11:8100`、ASR 仍请求 loopback。现已一次性同步模板、部署配置写入、激活 guard、计划和测试到固定实际 activity URL，该修复的 CI 与生产重跑继续由同一交付完成。
