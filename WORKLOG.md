...
- 验证：激活、既有部署和模型缓存专项测试 `42 passed`；本地可运行的 ASR/Provider 回归 `146 passed`；Python `py_compile`、PowerShell AST 解析、WORKLOG 标题格式和 `git diff --check` 通过；PR #19～#23 各七个 CI job 全部通过。生产 preflight 已依次验证并修复 Windows checkout、受控 GPU probe Token、防火墙规则归属判断和 PowerShell 原生 stderr；最新管理员 runner 执行中 health 已可用，但首次 capabilities 的 10 秒请求超时触发回滚，Scheduled Task 注销后底层 Uvicorn 子进程仍监听 8200。经管理员按固定路径与完整命令行确认后终止该进程，验证任务/规则/端口均不存在且配置为 false；现已将 capabilities 冷启动上限设为 120 秒，并在回滚中仅精确匹配时终止 ASR 子进程，该修复的 CI 与生产重跑继续由同一交付完成。
