...
- 验证：激活、既有部署和模型缓存专项测试 `42 passed`；本地可运行的 ASR/Provider 回归 `146 passed`；Python `py_compile`、两份 PowerShell AST 解析、WORKLOG 标题格式和 `git diff --check` 通过；PR #19、#20 各七个 CI job 全部通过。首次生产 run 在实际激活前连续两次因 Windows runner 直连 GitHub checkout 失败，代理修复后 checkout 通过；随后 preflight 发现 ASR env 的 GPU probe Token 为空，服务、任务和防火墙仍未改变。已在同一范围内复用既有仓库级 `GPU_SERVICE_TOKEN` Secret，通过受保护配置写入补齐 probe Token 且禁止输出；该修复的 CI 与生产重跑继续由同一交付完成。
