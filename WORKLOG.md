...
- 验证：激活、既有部署和模型缓存专项测试 `42 passed`；本地可运行的 ASR/Provider 回归 `146 passed`；Python `py_compile`、两份 PowerShell AST 解析、WORKLOG 标题格式和 `git diff --check` 通过；PR #19 七个 CI job 全部通过。首次生产 run 在实际激活前连续两次因 Windows runner 直连 GitHub checkout 失败，已在同一范围内让 Windows checkout 使用既有 Environment 代理，并使跨节点失败回滚改用受保护的本地脚本、不再依赖 checkout；该修复的 CI 与生产重跑继续由同一交付完成。
