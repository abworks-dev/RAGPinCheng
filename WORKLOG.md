...
### 06:19 — 保留 Qwen 依赖冲突证据并收敛脱敏

- 完成：修复 Qwen R3 依赖诊断对无版本 `depends on oss2` 行的识别，保留受控的依赖拥有者、依赖 specifier 和请求约束字段；诊断 schema 升级为 `/3`，继续排除路径、代理和原始日志内容。
- 文件：`scripts/qualify-qwen3-asr-production.ps1`、`tests/test_asr_deployment_static.py`、`WORKLOG.md`
...
### 07:08 — 补齐 Qwen aliyun core wheel blocker

- 完成：R3 run `31282600562` 确认 `aliyun-python-sdk-core==2.16.0` 仅发布 sdist；新增固定来源 wrapper，复用受控 wheel 合同接入该包，并修正兼容 `>=`/精确 pin 上下文不再误报为版本冲突。
- 文件：`scripts/build_internal_aliyun_core_wheel.py`、`.github/workflows/qualify-qwen3-asr-production.yml`、`scripts/qualify-qwen3-asr-production.ps1`、`tests/test_asr_deployment_static.py`、`WORKLOG.md`
...
### 13:37 — 统一三引擎 qualification 语料契约

- 完成：沿用 faster-whisper 已 PASS 八样本和历史 `sample_set_id=self-made-faster-whisper-r3`，新增严格只读的 `asr-qualification-corpus/1` loader；统一 faster-whisper、Qwen3-ASR、WhisperX runner/wrapper 对 manifest、WAV 身份和来源声明的校验，并保留各引擎报告、运行目录、依赖和模型隔离。三个 qualification workflow 增加与真实推理互斥的脱敏 manifest preflight，优先读取两个中性变量；迁移期保留引擎旧变量回退，新旧 manifest SHA-256、sample set 或 annotation version 不一致时失败关闭。WhisperX 诊断 workflow 不再生成 Qwen 样本。所有 Profile admission 保持原状态。
