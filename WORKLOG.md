...
- 待办/风险：严格单 Profile 契约修正尚待本地验证、PR、远端 CI、合并、自动部署及用户已授权的生产资格重跑。Profile admission 保持 disabled；不删除 Qwen3-ASR/WhisperX 模型，不重新下载或修改 faster-whisper 模型制品。回滚可 revert 单 Profile 契约提交；既有失败仅保留 run 专属资格目录，未执行未批准清理。
