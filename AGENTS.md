...
- 任何写任务必须使用同一仓库正式注册的 linked worktree，并在编辑前运行 `pwsh -NoProfile -File scripts/Test-CodexWorkspace.ps1 -Mode Write`。主 worktree 即使当前不在 `master` 也不得用于写任务。
- 写任务默认使用 `codex/*` 分支。恢复、发布或用户明确指定的非 `codex/*` 分支只有在说明理由并取得对应授权后，才可使用预检脚本的 `-AllowNonCodexBranch` 例外；该例外不放宽其他项目或风险边界。
...
- 预检脚本只提供诊断和门禁，不会创建、切换、清理、stash、提交或删除任何 Git 状态。预检失败时必须停止编辑并报告原因，不得绕过脚本结果继续执行。
