...
- 当本项目规则与通用 skill 的“等待计划确认”或“选择执行方式”要求冲突时，以本项目规则和用户已经给出的实施授权为准。用户明确批准方案后，Agent 不得因为 skill 要求再次选择执行方式、再次确认计划或等待新的“开始实施”回复；应直接执行已批准范围，除非用户缩小范围、暂停任务，或出现真实的权限、工具、外部服务或必要信息阻塞。
- Agent 不得将 token、上下文长度、响应时间、模型调度或其他内部限制描述为项目阻塞。若无法完成全部范围，必须报告实际未完成项和可复核原因，不得使用内部限制替代项目事实。
