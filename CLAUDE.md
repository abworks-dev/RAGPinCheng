- # RAGPinCheng Claude Code 项目入口
  ...
  功能地图描述”当前系统是什么”，`project-docs/decisions/`描述”为何作出已批准的长期选择”，`TODO.md`描述未来计划，`WORKLOG.md`描述实际完成记录；四者不得互相替代。
  ...
  涉及用户可观察行为变化的功能，Agent 完成技术验证后必须提供用户可执行的验收步骤，至少包含前置条件、测试数据、操作步骤、预期结果、异常检查和安全清理方式。此时状态写为“代码完成，待用户验收”；只有用户明确确认后才能写成“用户验收通过”。
  ...
  1. 使用本地日期（`Asia/Shanghai`），在 `## YYYY-MM-DD` 标题下追加本次记录；当天标题已存在时不得重复创建；
  2. 每次任务写一个 `### HH:mm — 简短任务名` 小节；时间不确定时可省略时间，只保留任务名；
  3. 只记录本次实际完成的工作，不复述完整对话，不把计划、尝试、猜测或未完成项写成成果；
  4. 必须列出修改内容、涉及文件、验证结果；存在未完成事项或风险时一并写明，没有则省略；
  5. 纯咨询、解释或只读调查若没有改变仓库内容，也要简短记录结论，并标注“未修改代码”；
  6. 同一任务后续补充修改时，优先更新原小节，避免重复记账；只有独立的新任务才新增小节；
  7. 日志不得包含密钥、密码、Cookie、个人信息、用户对话原文、客户资料或大段命令输出；
  8. 更新日志后，在最终回复中明确说明已记录到 `WORKLOG.md`；
  9. 若因权限、文件冲突或用户明确要求而无法更新，必须在最终回复中说明原因，不得静默跳过。
