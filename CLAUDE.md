...
除下方第 12 项的纯生产验收例外外，每个端到端任务或已批准执行阶段实际完成后、发送最终回复前，必须更新仓库根目录的 `WORKLOG.md`。同一阶段中的实现、CI 修复、合并、workflow 重跑和验收视为一个任务，在阶段收口时统一记录，不为中间进度或失败尝试重复建项。
...
12. 日志优先随当前功能提交或同一交付 PR 更新。合并后执行的纯生产验收若已有不可变 workflow/审计记录，不得仅为一条日志创建独立 PR 并触发全量 CI；可在下一次正常仓库交付中补记，并在当前最终回复中明确审计入口和待补记状态。
