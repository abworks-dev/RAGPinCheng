- # RAGPinCheng Claude Code 项目入口
  ...
  如果任务涉及 TODO.md 中列出的待办项，还需同步更新 `TODO.md`：将已完成项从待办区移到"✅ 最近已完成"列表，并补充完成日期。

  记录规则：

  1. 使用本地日期和任务完成时间（`Asia/Shanghai`）；日期标题统一为 `## YYYY-MM-DD`，任务标题统一为 `### HH:mm — 简短任务名`；
  ...
  9. 写入后检查日期标题唯一、任务标题格式、日期与同日时间顺序、小节边界；
