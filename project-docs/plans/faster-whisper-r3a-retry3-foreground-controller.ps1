...
function Invoke-SelfTest {
  ...
  $temp = Join-Path ([IO.Path]::GetTempPath()) ('r3a-retry3-controller-selftest-{0}' -f [Guid]::NewGuid().ToString('N'))
  ...
  Write-Output ('R3A_RETRY3_CONTROLLER_SELFTEST=' + ($result | ConvertTo-Json -Compress -Depth 6))
...
try {
  ...
  $controllerHash = Assert-FileHash $PSCommandPath $ExpectedControllerSha256 'retry3 foreground controller'

  $runFull = ''
  if (-not [string]::IsNullOrWhiteSpace($RunRoot)) {
    $runFull = Assert-DirectChild $RunRoot $AllowedRunParent 'RunRoot'
    if ((Split-Path -Leaf $runFull) -notmatch '^phase0-fw-r3a-retry-\d{8}-\d{6}$') { throw 'invalid retry RunRoot identity' }
  }
  ...
  $baseStatus = [ordered]@{
    schema_version='faster-whisper-r3a-retry3-foreground-controller/1'
    mode=$Mode
    status='starting'
  ...
  if ($Mode -eq 'ProbeSuccess') {
    ...
    $payload = "Write-Output 'R3A_RETRY3_PROBE_SUCCESS'; Start-Sleep -Milliseconds 250; exit 0"
  } elseif ($Mode -eq 'ProbeTimeout') {
    if ($ChildTimeoutSeconds -gt 10) { throw 'ProbeTimeout timeout must be at most 10 seconds' }
    $payload = "Write-Output 'R3A_RETRY3_PROBE_TIMEOUT_STARTED'; Start-Sleep -Seconds 120; exit 0"
  } else {
    foreach ($pair in @(
    ...
    Assert-FileHash $RetryPlanPath $ExpectedRetryPlanSha256 'retry3 plan' | Out-Null
  ...
  if ($Mode -eq 'ProbeSuccess') {
    $ok = (-not $result.timed_out -and $result.exit_code -eq 0 -and $result.stdout -match 'R3A_RETRY3_PROBE_SUCCESS')
    ...
    if (-not $result.timed_out -and $result.exit_code -eq 0 -and $missing.Count -eq 0) {
      ...
      if (Test-Path -LiteralPath $runFull -PathType Container) {
        ...
        if (-not (Test-Path -LiteralPath $stopPath -PathType Leaf)) {
          $stop = @(
            '# faster-whisper R3-A retry3 automatic stop','',
  ...
  if (-not [string]::IsNullOrWhiteSpace($runFull) -and (Test-Path -LiteralPath $runFull -PathType Container)) {
    $runEvidence = Join-Path $runFull 'evidence\retry3-foreground-controller-status.json'
    Write-AtomicJson $runEvidence $terminal
  }
  Write-Output ('R3A_RETRY3_CONTROLLER_RESULT=' + ($terminal | ConvertTo-Json -Compress -Depth 12))
  exit $controllerExit
} catch {
