...
function Invoke-SelfTest {
  ...
  $temp=Join-Path ([IO.Path]::GetTempPath()) ('faster-whisper-r3a-retry4-selftest-{0}' -f [Guid]::NewGuid().ToString('N'));New-Item -ItemType Directory -Path $temp -Force|Out-Null
  try {
    ...
    $gui=[pscustomobject]@{ProcessId=100;Name='explorer';ExecutablePath='C:\Windows\explorer.exe';CommandLine=''};$gateGui=Get-AsrProcessGate @() @($gui) @() 'E:\FunASR-Phase0\faster-whisper-runs\phase0-fw-r3a-retry4-test' 999;Check (-not $gateGui.blocked) 'ordinary GUI process incorrectly blocked A1';$gateActive=Get-AsrProcessGate @('E:\FunASR-Phase0\active-run.json') @($gui) @() 'E:\FunASR-Phase0\faster-whisper-runs\phase0-fw-r3a-retry4-test' 999;Check $gateActive.blocked 'active-run did not block A1'
    $identity=[pscustomobject]@{exists=$true;process_id=101;command_line='python.exe E:\FunASR-Phase0\faster-whisper-runs\phase0-fw-r3a-retry4-test\smoke.py --model faster-whisper'};$exact=[ordered]@{process_id=101;status='ok';blocker=$null;identity=$identity;watchdog=$null};Check ((Get-AsrProcessGate @() @($gui) @($exact) 'E:\FunASR-Phase0\faster-whisper-runs\phase0-fw-r3a-retry4-test' 999).blocked) 'exact run-bound identity did not block A1';$blockedQuery=[ordered]@{process_id=102;status='timeout';blocker='WMI_EXACT_QUERY_TIMEOUT';identity=$null;watchdog=$null};Check ((Get-AsrProcessGate @() @($gui) @($blockedQuery) 'E:\FunASR-Phase0\faster-whisper-runs\phase0-fw-r3a-retry4-test' 999).blocked) 'WMI timeout blocker did not block A1'
  ...
  $result=[ordered]@{tests=$counter[0];failures=@($failures);passed=($failures.Count-eq 0);real_wmi_invoked=$false;real_nvidia_smi_invoked=$false};Write-Output ('R3A_RETRY4_SELFTEST='+($result|ConvertTo-Json -Compress -Depth 8));if($failures.Count-gt 0){exit 99};exit 0
}
...
try {
  ...
  $dateConsistency=('Window and reports use Asia/Shanghai +08:00; local start date={0}; local end date={1}; UTC start date={2}; UTC end date={3}.' -f $start.ToString('yyyy-MM-dd'),$end.ToString('yyyy-MM-dd'),$start.UtcDateTime.ToString('yyyy-MM-dd'),$end.UtcDateTime.ToString('yyyy-MM-dd'))
  ...
  if($runId-notmatch '^phase0-fw-r3a-retry4-\d{8}-\d{6}$'){throw "invalid retry run id: $runId"}
  ...
  Assert-FileHash $PSCommandPath $ExpectedHelperSha256 'retry4 A0/A1 helper'|Out-Null
  ...
  $createdRun=$true
  $retrySnap=Join-Path $runFull 'evidence\faster-whisper-r3a-retry4-execution-plan.md'
  ...
  $helperSnap=Join-Path $runFull 'helpers\faster-whisper-r3a-retry4-a0-a1.ps1'
  ...
  $baseline=[ordered]@{
    schema_version='faster-whisper-r3a-retry4-preflight/1';run_id=$runId;run_root=$runFull;collected_at=($now.ToString('o'));hostname=$env:COMPUTERNAME
    os=$os
  ...
  $approval=[ordered]@{
    schema_version='faster-whisper-r3a-retry4-approval/1';approved_retry_plan_sha256=($ExpectedRetryPlanSha256.ToLowerInvariant());original_plan_sha256=$ExpectedOriginalPlanSha256
  ...
  $config=[ordered]@{
    schema_version='faster-whisper-r3a-retry4-config/1';run_id=$runId;run_root=$runFull
  ...
  $manifestPath=Join-Path $runFull 'state\helper-manifest.json';Write-JsonFile $manifestPath ([ordered]@{schema_version='faster-whisper-r3a-retry4-helper-manifest/1';created_at=([DateTimeOffset]::Now.ToString('o'));helpers=$helperEntries})
  $identity=[ordered]@{
    schema_version='faster-whisper-r3a-retry4-run-identity/1';run_id=$runId;run_root=$runFull;created_at=([DateTimeOffset]::Now.ToString('o'))
  ...
  $preflight=@(
    '# faster-whisper R3-A retry4 P1 preflight','',('- Run: `{0}`'-f $runFull),('- Collected: `{0}`'-f $now.ToString('o')),
