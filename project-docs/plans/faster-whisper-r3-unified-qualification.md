...
`D:\ServiceData\RAGPinCheng-ASR\qualification\wheel-cache`，按 Python 版本与 ABI、
...
`D:\RAGPinCheng\runtime\qualification-inputs\faster-whisper\jieba-0.42.1.tar.gz`。
...
```text
Windows GPU / ASR: 192.168.11.11
Ubuntu backend:    192.168.11.12
GPU service:       RAGPinCheng-GPU / TCP 8100
ASR service:       RAGPinCheng-ASR / TCP 8200
GitHub runner:     FJPCSEVER / Administrator / asr-production
ASR program root:  D:\Services\RAGPinCheng-ASR
ASR data root:     D:\ServiceData\RAGPinCheng-ASR
```
...
```text
资格程序根：
D:\Services\RAGPinCheng-ASR\qualification\faster-whisper

每次运行：
D:\Services\RAGPinCheng-ASR\qualification\faster-whisper\runs\<github.run_id>\

固定样本入口：
D:\ServiceData\RAGPinCheng-ASR\qualification\faster-whisper\inputs\

固定样本 Manifest：
D:\ServiceData\RAGPinCheng-ASR\qualification\faster-whisper\inputs\manifest.json

模型最终目录：
D:\ServiceData\RAGPinCheng-ASR\models\faster-whisper-large-v3-turbo\
...
- TCP 8100/8200 防火墙；
- 当前 ASR 生产 venv；
- `D:\ServiceData\RAGPinCheng-ASR\config\asr.env`；
- Ubuntu `prod.env`；
- Ubuntu `ASR_ENABLED`；
- backend 容器。
