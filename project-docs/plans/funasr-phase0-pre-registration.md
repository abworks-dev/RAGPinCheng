...
- 唯一一台 RTX 5060 Ti 位于公司生产 Windows GPU 主机（参见 `project-docs/migrations/ubuntu-app-windows-gpu-runbook.md` §1、`scripts/deploy-gpu.ps1` §6 中 `HOST=192.168.11.11` / `PORT=8100`），当前承载实际 `gpu_service`。
...
- **隔离边界**：
  - venv：`E:\FunASR-Phase0\.venv`（与本仓库 `.venv` 完全隔离，永不合并）；
  - 模型缓存：`E:\FunASR-Phase0\models`（HF / ModelScope / funasr cache）；
  - 测试音频与提取中间产物：`E:\FunASR-Phase0\testdata`；
  - 标注：`E:\FunASR-Phase0\annotations`（UTF-8、JSONL+CSV 双格式）；
  - 日志：`E:\FunASR-Phase0\logs`（按 `short/`、`1h/`、`2h/`、`4h/`、`bge-coexist/` 子目录分桶）；
  - 报告：`E:\FunASR-Phase0\reports\phase0-report.md`（结构化 Markdown + 原始数据 `metrics.csv` + 评估脚本 `eval.py`）；
...
- 模型缓存：`E:\FunASR-Phase0\models\…`（与生产 `hf_cache` 卷隔离，不写入）。
- 临时音频与抽取 WAV：`E:\FunASR-Phase0\testdata\extracted\`；任务完成后立即删除，但 `audio_sha256` 保留在 `metrics.csv`。
- 报告：`E:\FunASR-Phase0\reports\phase0-report.md` + `metrics.csv` + `license-matrix.md` + `raw-logs/`。
- **清理方式**：
  - 测试全部完成后：`rm -rf E:\FunASR-Phase0\models E:\FunASR-Phase0\.venv E:\FunASR-Phase0\testdata\extracted`；
  - 报告与标注保留 90 天（默认 `E:\FunASR-Phase0\reports\archive\`），到期后由本人手动清理；
  - 若需在 30 天内删除报告与缓存，须走 R3 删除审批。
- **磁盘上限**：30 GB 硬上限，25 GB 软警告；超阈值时停止下载并报告。
- **失败恢复**：失败时只清缓存和临时音频；保留 `raw-logs/` 与 `metrics.csv` 以供复盘。
- **回滚**：Phase 0 沙箱完全自包含；不影响任何业务、数据库、索引、生产服务；删除 `E:\FunASR-Phase0` 目录即视为 Phase 0 全部回滚完成。
...
| 报告模板 | `E:\FunASR-Phase0\reports\phase0-report.md`（由评测脚本生成） | Markdown | 实际测试时 |

> 上述 `scripts/funasr_phase0/*` 文件**仅在用户批准 Phase 0 后才落地**；本轮不创建。

## 15. 风险与回滚（本轮范围）

- **回滚**：`E:\FunASR-Phase0` 自包含；本轮范围内不创建任何文件，因此**回滚成本 = 0**。
