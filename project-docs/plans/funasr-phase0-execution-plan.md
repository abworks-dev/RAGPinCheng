...
- 下载范围：允许批准的 PyTorch 索引、PyPI 镜像与 ModelScope；模型和缓存只能进入 `E:\FunASR-Phase0\models`。
...
|---|---|---|---|
| Python 解释器 | `C:\Program Files\Python310\python.exe`（见 `scripts/deploy-gpu.ps1` §5） | 新建 `C:\FunASR-Phase0\venv\python.exe`（基于 `C:\Python311\python.exe -m venv`） | **不同解释器路径**，互不污染 |
| 进程 | `python -m gpu_service.app` 后台进程 | `python -m funasr_phase0.runner ...` 前台/可终止 | **独立进程**，不 `import gpu_service` |
| 工作目录 | `E:\Repository\Github\RAGPinCheng\gpu_service` | `E:\FunASR-Phase0\runner` | 物理目录分离 |
...
| 模型缓存 | `gpu_service/.cache/huggingface`（生产） | `E:\FunASR-Phase0\models`（`HF_HOME` / `MODELSCOPE_CACHE` 指向） | 不读生产缓存；不复用生产权重 |
...
E:\FunASR-Phase0\models\          # HF_HOME / MODELSCOPE_CACHE / funasr cache
E:\FunASR-Phase0\testdata\        # 测试样本与抽取音频
E:\FunASR-Phase0\annotations\      # 人工标注 JSONL/CSV
E:\FunASR-Phase0\logs\            # raw-logs/
E:\FunASR-Phase0\reports\         # 阶段报告 + metrics.csv
E:\FunASR-Phase0\runner\          # 入口脚本与评测脚本
```
...
# 3) 1 分钟平均利用率
nvidia-smi dmon -s pucm -c 60 -d 1 | Tee-Object E:\FunASR-Phase0\logs\baseline-gpu-dmon.txt
```
...
```powershell
# 1) /health
curl -s -H "Authorization: Bearer $env:GPU_SERVICE_TOKEN" http://192.168.11.11:8100/health
# 2) /model-info
curl -s -H "Authorization: Bearer $env:GPU_SERVICE_TOKEN" http://192.168.11.11:8100/model-info
```
...
- 记录 `p50 / p95 / p99 / max / 错误率`，写到 `E:\FunASR-Phase0\logs\baseline-bge-latency.csv`。
...
- **通道 A**：Windows OpenSSH（已开 `PasswordAuthentication no`，默认走密钥）。请告知：
  - 我能否以当前 Windows 用户 `ASUS` 凭私钥登录？
  - 若需在 `192.168.11.11` 放置公钥，请把授权公钥路径告知。
...
- 最终报告：`E:\FunASR-Phase0\reports\phase0-report.md` + `metrics.csv` + `license-matrix.md` + `raw-logs/`；
...
### R3-5 失败样本诊断约束（2026-08-01）

- 首轮八样本结果为 7/8，通过项不得因诊断重跑；`noise_with_bim` 的 CER 与 BIM 术语召回未达预注册阈值，禁止事后放宽阈值或修改 reference。
- 仅可对明确标记为自制、非内部录音的目标样本启用显式正文诊断；默认报告和所有 checkpoint 继续不保存 reference/hypothesis 正文。
- 诊断执行仍须通过 `00_run_guarded.py`、本地模型离线门禁、许可门禁、维护窗口、BGE 监控与恢复检查；诊断结果不自动授权热词、1h 或后续测试。
