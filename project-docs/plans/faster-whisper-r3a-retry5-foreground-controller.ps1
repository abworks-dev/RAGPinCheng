...
$AllowedStagingParent = 'E:\FunASR-Phase0\faster-whisper-inputs'
...
function Get-FileLength([string]$Path,[string]$Label) {
...
function Invoke-SelfTest {
  ...
  $temp = Join-Path ([IO.Path]::GetTempPath()) ('r3a-retry5-controller-selftest-{0}' -f [Guid]::NewGuid().ToString('N'))
  ...
  Write-Output ('R3A_RETRY5_CONTROLLER_SELFTEST=' + ($result | ConvertTo-Json -Compress -Depth 6))
...
$baseStatus = $null
try {
  foreach ($pair in @(@('Mode',$Mode),@('StagingRoot',$StagingRoot))) {
  ...
  $controllerBytes = Get-FileLength $PSCommandPath 'retry5 foreground controller'

  $runFull = ''
  if (-not [string]::IsNullOrWhiteSpace($RunRoot)) {
    $runFull = Assert-DirectChild $RunRoot $AllowedRunParent 'RunRoot'
    if ((Split-Path -Leaf $runFull) -notmatch '^phase0-fw-r3a-retry5-\d{8}-\d{6}$') { throw 'invalid retry RunRoot identity' }
  }
  ...
  $baseStatus = [ordered]@{
    schema_version='faster-whisper-r3a-retry5-foreground-controller/1'
    mode=$Mode
    status='starting'
  ...
  if ($Mode -eq 'ProbeSuccess') {
    ...
    $payload = "Write-Output 'R3A_RETRY5_PROBE_SUCCESS'; Start-Sleep -Milliseconds 250; exit 0"
  } elseif ($Mode -eq 'ProbeTimeout') {
    if ($ChildTimeoutSeconds -gt 10) { throw 'ProbeTimeout timeout must be at most 10 seconds' }
    $payload = "Write-Output 'R3A_RETRY5_PROBE_TIMEOUT_STARTED'; Start-Sleep -Seconds 120; exit 0"
  } else {
    foreach ($pair in @(
      @('RunRoot',$runFull),@('HelperPath',$HelperPath),@('RetryPlanPath',$RetryPlanPath),
      @('OriginalPlanPath',$OriginalPlanPath),@('StaticPrecheckPath',$StaticPrecheckPath),
      @('BgeHelperPath',$BgeHelperPath),@('SamplePath',$SamplePath)
    )) { Require-Value $pair[0] $pair[1] }
    if (Test-Path -LiteralPath $runFull) { throw "RunRoot already exists; new identity required: $runFull" }
    [void](Get-FileLength $HelperPath 'A0/A1 helper')
    [void](Get-FileLength $RetryPlanPath 'retry5 plan')
    [void](Get-FileLength $OriginalPlanPath 'original R3-A plan')
    [void](Get-FileLength $StaticPrecheckPath 'static precheck')
    [void](Get-FileLength $BgeHelperPath 'BGE helper')
    [void](Get-FileLength $SamplePath 'approved synthetic sample')
    $invoke = @(
  ...
  if ($Mode -eq 'ProbeSuccess') {
    $ok = (-not $result.timed_out -and $result.exit_code -eq 0 -and $result.stdout -match 'R3A_RETRY5_PROBE_SUCCESS')
    ...
    if (-not $result.timed_out -and $result.exit_code -eq 0 -and $missing.Count -eq 0) {
      ...
      if (Test-Path -LiteralPath $runFull -PathType Container) {
        ...
        if (-not (Test-Path -LiteralPath $stopPath -PathType Leaf)) {
          $stop = @(
            '# faster-whisper R3-A retry5 automatic stop','',
  ...
  if (-not [string]::IsNullOrWhiteSpace($runFull) -and (Test-Path -LiteralPath $runFull -PathType Container)) {
    $runEvidence = Join-Path $runFull 'evidence\retry5-foreground-controller-status.json'
    Write-AtomicJson $runEvidence $terminal
  }
  Write-Output ('R3A_RETRY5_CONTROLLER_RESULT=' + ($terminal | ConvertTo-Json -Compress -Depth 12))
  exit $controllerExit
} catch {
