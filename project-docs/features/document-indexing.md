...
管理员可以通过既有资料管理页上传并索引 PDF、Markdown、DOCX、XLSX、PPTX 和教学视频转录稿。受管资料库功能开启后，具有独立资料权限的人员还可以按“整理上传 → 负责人确认 → 管理员发布”处理普通资料；分类、业务资料、版本、审核、发布和正式 head 由 `app.sqlite` 管理，只有发布成功的当前版本进入正式检索。
...
- 管理端资料总览、搜索、分类/类型/状态筛选、分页、源文件预览、视频与转写稿预览、重试、重新索引与安全删除；
