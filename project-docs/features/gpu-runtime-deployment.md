...
由于 Helios 经部署代理访问官方 PyTorch wheel 重定向域名会出现 TLS EOF，R3-2B 使用人工下载的单一 CUDA Torch wheel 作为受控输入。管理员在可直连官方站点的电脑下载精确文件 `torch-2.7.0+cu128-cp310-cp310-win_amd64.whl`，运行 `scripts/new-gpu-torch-wheel-seed-manifest.ps1` 生成同目录 `manifest.json`，再将两个文件传入 `${PRODUCTION_REPO_PATH}\runtime\wheel-seed\torch-2.7.0-cu128-cp310-win_amd64\`。校验器固化PyTorch官方索引公布的SHA-256 `c52c4b869742f00b12cb34521d1381be6119fa46244791704b00cc4a3cb06850`；Helios只接受精确文件集合、Python 3.10/`win_amd64`标签、官方cu128索引标识、文件长度和发布者哈希均匹配的目录，校验失败不会安装。
