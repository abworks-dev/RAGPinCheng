...
- Ubuntu 主机：
  ...
  - 主机名：`Bim-admin`；
  - 内网地址：`192.168.11.12/24`；
  - 网关：`192.168.11.1`；
  - 无 NVIDIA GPU；
  - SSH 公网入口为 `www.bimtrans.top:2222`，路由器转发到主机 TCP 22；
...
- Windows 防火墙仅允许 `192.168.11.12` 访问；
...
- 固定 Ubuntu 地址 `192.168.11.12`；
- 安装并固定 Docker/Compose 版本；
- 准备仓库、数据、备份和日志目录；
- 配置磁盘容量告警；
- 配置 HTTPS 反向代理；
- 仅暴露 HTTPS 与获批 SSH；
- RDP 3389/3390 仅允许 `192.168.11.0/24`；
- 保留向日葵并验证防火墙启用后的连接；
- Qdrant 6333 不对公网开放；
- 配置时间同步；
- 设置自动安全更新策略和维护窗口；
...
| 网络 | GPU 端口仅允许 `192.168.11.12`，公网不可达 |
...
详细用户验收格式遵循 `project-docs/USER_ACCEPTANCE.md`。
...
9. 有用户可观察变化时提供 `project-docs/USER_ACCEPTANCE.md` 格式的验收步骤；
10. 按 `CLAUDE.md` 规则更新 `WORKLOG.md`；
...
| # | 问题 | 决策 |
|---|------|------|
| 1 | GPU 认证方式 | API Token + Windows 防火墙（仅允许 `192.168.11.12` 访问） |
...
- [x] 确认 Windows 内网 IP — `192.168.11.11`
