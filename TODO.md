...
- 下一步：
  - [ ] 重新转录受影响的培训视频与 ASR 验收媒体，在工作台确认新稿包含开场白、讲解连续、无整段规范编号回吐、无字幕署名幻觉（「中文字幕志愿者 …」）、无「楼坯/结枅/梨柱/平态/结购」类错字（同一 120 秒窗口：塌缩时 6 段/72 字 → 去 rp 后 12 段/244 字 → 开启 VAD 后 27 段/412 字；整片 1012 → 2022 → 2428 字）。
  - [ ] 抽查含回吐历史版本的视频，确认发布被拦截并提示重新转录生成新版本。
  - [ ] 决策「热词压制内容」这一结构性矛盾（A 路线当前被阻塞）：实测在 qualification 语料上，无热词配置会丢掉规范编号样本的 `50016`（只剩 `2014`）且两个 noisy-BIM 术语全部丢失，因此去掉热词**无法通过认证**；但真实长音频上热词又不产出正确编号、反而制造不存在的版本号（GB 50011-2014）并压掉 40 至 60% 内容。解法是**重做语料**：补一条真实长音频中自然说出规范编号与 BIM 术语的样本，再评估无热词配置能否达到绝对门槛；在此之前保持热词不动。典型残留即 11:49 处「小于三百的时候」之后紧跟 `建筑设计规范 GB 50011-2010` 并出现约 41 秒静音段丢内容：文本层无法与"真实引用结尾"区分（门禁本身奖励保留编号），必须靠解码层（去热词）解决。
  - [ ] 为 WhisperX qualification 语料补一条 60 至 120 秒长音频样本并重跑认证：现语料仅 3.5 至 5.2 秒，实测短片段上 `repetition_penalty` 2.0 与 1.0 产出几乎一致（23 字比 21 字），无法覆盖长音频塌缩、热词复读、静音段丢内容与 VAD 收益。
  - [ ] 持续关注 ASR 节点 D: 空间：已按策略回收 8 个退役 release 与两棵退役依赖树（清单见节点 `release-retirement.json`），剩余可回收项为超 7 天保留期的 `dependency-runs`。
...
- 完成标准：批量转录不再超时且转录稿完整、无复读、无整段热词回吐、无字幕署名幻觉与术语错字；单个畸形对齐片段不会导致整单失败；WhisperX profile 出稿量与语速匹配；含回吐内容的版本无法发布；已删除版本的转录片段不再出现在检索结果中；用户确认验收通过。
- 依赖：ASR active release `34709139061`（whisperx 资格 run `34708638763`、契约 `962dcb560a9c98c2f90b9e95ef8ed6d3f4e283e1a07aae0076ab7d34c809e592`；引擎为 VAD 开启 + 无 `repetition_penalty` + `no_repeat_ngram_size=3` + 越界对齐片段裁剪）与应用 master tip `946bb1d5`（已部署；术语层含 BIM 误识纠正、繁体归一、段内热词串清理与 Whisper 字幕署名清理）。
- 方案链接：`docs/features/transcript-pipeline.md`
