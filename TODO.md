...
- 下一步：
  - [ ] 重新转录受影响的培训视频与 ASR 验收媒体，在工作台确认新稿包含开场白、讲解连续且 11:49 之后不再成段缺失、无整段规范编号回吐、无字幕署名幻觉（「中文字幕志愿者 …」）、无「楼坯/结枅/梨柱/平态/结购」类错字（同一 120 秒窗口：塌缩时 6 段/72 字 → 去 rp 后 12 段/244 字 → 开启 VAD 后 27 段/412 字；整片 1012 → 2022 → 2428 字；690–840 秒区间覆盖 69.2% → 99.8%）。
  ...
  - [ ] 持续关注 ASR 节点 D: 空间（每次认证需约 16 GB、暂存约 25 GB）：已按策略回收退役 release、退役依赖树与失败/历史认证运行的临时目录（保留 reports 证据）。建议后续把认证失败的临时目录清理纳入自动 compaction，避免再次因磁盘满导致认证失败。
...
- 依赖：ASR active release `34715125611`（whisperx 资格 run `34713859346`、契约 `f49509120a97889bfe73986af2c37c385664776bccd8ac72b1e5052e83d484e7`；引擎为 VAD 开启 + 无 `repetition_penalty` + **无** `no_repeat_ngram_size` + 越界对齐片段裁剪）与应用 master tip `95e94263`（已部署；术语层含 BIM 误识纠正、繁体归一、段内热词串清理、Whisper 字幕署名清理与退化重复折叠）。
- 方案链接：`docs/features/transcript-pipeline.md`
