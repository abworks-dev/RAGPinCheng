...
- 下一步：
  - [ ] 重新转录受影响的培训视频与 ASR 验收媒体，在工作台确认新稿包含开场白、讲解连续且 11:49 之后不再成段缺失、无整段规范编号回吐、无成段复读（含 11:30 之后「楼层平台」类跨段复读）、无字幕署名幻觉（「中文字幕志愿者 …」）、无「楼坯/结枅/梨柱/平态/结购/进高/屏面」类错字（同一 120 秒窗口：塌缩时 6 段/72 字 → 去 rp 后 12 段/244 字 → 开启 VAD 后 27 段/412 字；整片 1012 → 2022 → 2428 字；690–840 秒区间覆盖 69.2% → 99.8%）。
  - [ ] 抽查含回吐历史版本的视频，确认发布被拦截并提示重新转录生成新版本。
  - [ ] 决策「热词压制内容」这一结构性矛盾（A 路线当前被阻塞）：实测在 qualification 语料上，无热词配置会丢掉规范编号样本的 `50016`（只剩 `2014`）且两个 noisy-BIM 术语全部丢失，因此去掉热词**无法通过认证**；但真实长音频上去掉热词可再多 26% 内容（同区间 444 → 559 字）且不产生填空段。解法是**重做语料**：补一条真实长音频中自然说出规范编号与 BIM 术语的样本，再评估无热词配置能否达到绝对门槛；在此之前保持热词不动。
  - [ ] 为 WhisperX qualification 语料补一条 60 至 120 秒长音频样本并重跑认证：现语料仅 3.5 至 5.2 秒，实测短片段上 `repetition_penalty` 2.0 与 1.0 产出几乎一致（23 字比 21 字），也测不出 3-gram 禁令造成的整段跳过，无法覆盖长音频类缺陷。
  - [ ] 持续关注 ASR 节点 D: 空间（每次认证需约 16 GB、暂存约 25 GB）：已按策略回收退役 release、退役依赖树与失败/历史认证运行的临时目录（保留 reports 证据）；同时修复了 `cleanup-asr-storage-production.yml` 从未真正可用的两处缺陷（`$LASTEXITCODE` 恒判失败、批准摘要含生成时间戳导致 apply 必然拒绝、删除循环在点源调用下 `$PSCmdlet.ShouldProcess` 抛 NullReferenceException），本轮已用它回收 18.84 GB（D: 45.51 → 59.68 GB）。建议后续把认证失败的临时目录清理纳入自动 compaction，避免再次因磁盘满导致认证失败。
...
- 依赖：ASR active release `34720042701`（whisperx 资格 run `34719564190`、契约 `106eabb3e36b8979d4a953b5d1791c2d8163e2a0abbef8a6c21dbd534c0d4f27`；引擎为 VAD 开启 + 无 `repetition_penalty` + **无** `no_repeat_ngram_size` + 越界对齐片段裁剪）与应用部署 `535c6fa4`（整轮交付的 master tip 为 `423b86b9`，其后提交仅含 workflow/测试），术语层含 BIM 误识纠正、繁体归一、段内热词串清理、Whisper 字幕署名清理、退化重复折叠（含带 `-./%` 的规范编号与长短语成对复读）以及跨段短句复读护栏（连续 ≥3 段相同短文本、间隔 ≤15 秒只保留首段）。本轮生产验证：应用容器内 15 例只读术语诊断 `status=pass`、`failed_count=0`（含整段伪造编号复读剔除、成对编号剔除、`进高→净高`/`屏面→平面` 与护栏常量）。
- 方案链接：`docs/features/transcript-pipeline.md`
