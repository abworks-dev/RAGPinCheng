...
### 7-R. 检索黄金集重建（Golden Set Rebuild）— ✅ 第一期完成，用户验收通过（2026-07-30）

**优先级**：🔴 高（阻塞 Phase 2 查询拆分收益量化——commit 9bf065a 默认关闭，开启前需要能量化收益的黄金集）

**状态**：✅ 第一期完成并**用户验收通过**（2026-07-30）。新 `golden.jsonl` 75 题已入库（commit 553a802），生产新基线 R@1=75.4%/R@5=100%/MRR@5=0.870、no_answer 6/6=100%。剩余为可选增量：阶段3 索引指纹防陈旧、阶段5 对比型用例、公司流程类第二期评测（均未启动，非阻塞）。
...
- [x] **阶段 3 完成(2026-07-30,commit 240cb29)——索引指纹防陈旧机制**：`src/eval/fingerprint.py`(compute/load/write/compare)+ `src/eval/golden.fingerprint.json`(基准 sidecar,parent_count=20088, sha256=8478af62..., frozen_at=2026-07-30T22:30+08:00)+ `run_eval_retrieval.py` 启动校验 + `--strict-staleness` 标志 + `relabel_golden.py fingerprint --freeze`。**生产三路径验证**:匹配→`staleness OK`;篡改 baseline→显眼 `!!! WARNING` 段(非阻断);`--strict-staleness`→SystemExit 2 阻断。篡改测试精准命中"只重解析(parent_id 哈希变,数量不变)"这种最隐蔽陈旧模式,告警明确告知"R@K≈0 是标注陈旧不是检索坏了"。方案文档 `project-docs/golden-set-staleness-guard.md`。
