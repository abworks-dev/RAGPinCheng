...
- 下一步：
  - [ ] 重新转录受影响的培训视频与 ASR 验收媒体，在工作台确认新稿包含开场白、讲解连续、无整段规范编号回吐、无「楼坯/结枅/梨柱」类错字（同一 120 秒窗口：塌缩时 6 段/72 字 → 去 rp 后 12 段/244 字 → 开启 VAD 后 27 段/412 字；整片 1012 → 2022 → 2428 字）。
  - [ ] 抽查含回吐历史版本的视频，确认发布被拦截并提示重新转录生成新版本。
  - [ ] 决策「热词压制内容」这一结构性矛盾（A 路线当前被阻塞）：实测在 qualification 语料上，无热词配置会丢掉规范编号样本的 `50016`（只剩 `2014`）且两个 noisy-BIM 术语全部丢失，因此去掉热词**无法通过认证**；但真实长音频上热词又不产出正确编号、反而制造不存在的版本号（GB 50011-2014）并压掉 40 至 60% 内容。解法是**重做语料**：补一条真实长音频中自然说出规范编号与 BIM 术语的样本，再评估无热词配置能否达到绝对门槛；在此之前保持热词不动。
...
- 完成标准：批量转录不再超时且转录稿完整、无复读、无整段热词回吐与术语错字；单个畸形对齐片段不会导致整单失败；WhisperX profile 出稿量与语速匹配；含回吐内容的版本无法发布；已删除版本的转录片段不再出现在检索结果中；用户确认验收通过。
- 依赖：ASR active release `34695977160`（whisperx 资格 run `34695443163`、契约 `09033305d7b1bb494660f9cbfeb10154da57e73ea35daccdfa9c39513d4fc1ec`；引擎为 VAD 开启 + 无 `repetition_penalty` + `no_repeat_ngram_size=3` + 越界对齐片段裁剪）与应用 master tip `63818ea3`（已部署；术语层含 BIM 误识纠正、繁体归一与段内热词串清理）。
- 方案链接：`docs/features/transcript-pipeline.md`
- 方案链接：`docs/features/transcript-pipeline.md`
