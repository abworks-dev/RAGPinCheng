...
- 状态：待用户验收
- 目标：修复生产批量转录超时与转录稿残缺（WhisperX 解码塌缩只转录约四分之一语音、单个越界对齐片段使整单永久失败、热词复读污染输出、关闭 VAD 丢语音、3-gram 禁令使解码跳过整段语音）、制度上杜绝含 ASR 提示词回吐的旧稿被发布，并清理已删除转录版本的检索索引残留。
- 下一步：
  - [ ] 重新转录受影响的培训视频与 ASR 验收媒体，在工作台确认新稿包含开场白、讲解连续且 11:49 之后不再成段缺失、无整段规范编号回吐、无成段复读（含 11:30 之后「楼层平台」类跨段复读）、无字幕署名幻觉（「中文字幕志愿者 …」）、无「楼坯/结枅/梨柱/平态/结购/进高/屏面」类错字（同一 120 秒窗口：塌缩时 6 段/72 字 → 去 rp 后 12 段/244 字 → 开启 VAD 后 27 段/412 字；整片 1012 → 2022 → 2428 字；690–840 秒区间覆盖 69.2% → 99.8%）。第 6 轮去热词后 2.4.10 楼梯专项检查的对应区间由「整段编号复读 248 字/54 字」恢复为 12 段 108 字与 7 段 84 字真实讲解（活动 release 内只读重放证据）。
  - [ ] 抽查含回吐历史版本的视频，确认发布被拦截并提示重新转录生成新版本。
  - [x] 决策「热词压制内容」这一结构性矛盾：2026-09-13 用户批准方案 B（WhisperX 生产**全局去热词**），并以「自然长音频」语料替代人工 5 秒念编号片段、把选择门禁改为「绝对门禁 + 对上一版热词配置的同口径回归比较」。faster-whisper / qwen3 的配置与语料保持 v1 不变。
  - [ ] 若验收时仍见个别词级误识（当前已覆盖 `静高/径高/净够→净高`、`构建碰撞/碰壮→构件碰撞`、`结构构建→结构构件`、`标柱→标注`、`铺面(视图|图)→剖面`；尚未覆盖实测出现的 `镜膏→净高`、`替兰→梯梁`，`极端→梯段` 因会改坏「极端情况」等正常表达而故意不加），再按实测样本批量补规则并重跑 whisperx 认证。
  - [ ] 持续关注 ASR 节点 D: 空间（每次认证需约 16 GB、暂存约 25 GB）：已按策略回收退役 release、退役依赖树与失败/历史认证运行的临时目录（保留 reports 证据）；同时修复了 `cleanup-asr-storage-production.yml` 从未真正可用的两处缺陷（`$LASTEXITCODE` 恒判失败、批准摘要含生成时间戳导致 apply 必然拒绝、删除循环在点源调用下 `$PSCmdlet.ShouldProcess` 抛 NullReferenceException），第 6 轮又用它回收 33.7 GB（D: 19.44 → 45.02 GB；fail-fast 的认证要求 ≥30 GiB 空闲）。建议后续把认证失败的临时目录清理纳入自动 compaction，避免再次因磁盘满导致认证失败。
...
- 完成标准：批量转录不再超时且转录稿完整、无成段缺失、无复读、无整段热词回吐、无字幕署名幻觉与术语错字；单个畸形对齐片段不会导致整单失败；WhisperX profile 出稿量与语速匹配；含回吐内容的版本无法发布；已删除版本的转录片段不再出现在检索结果中；用户确认验收通过。
- 依赖：ASR active release `34730270408`（whisperx 资格 run `34728572432`、契约 `a16311894a415a88a88f0557a57ba469944b467c17dc1c15e8437e26761b05ab`，activation `34731121441`；**生产解码已全局去热词**，beam 10 / temperature 0.0 / VAD 开启 / 无 `repetition_penalty` / 无 `no_repeat_ngram_size` / 越界对齐片段裁剪；faster-whisper 资格 run `34729671471`、契约 `a804faf92a9ae211cd416f49c98f6aab7d77df4355660f7a0d5711892478ff5e`）与应用部署 `2b705fdd`（术语层含第 4–6 轮全部纠正规则与跨段复读护栏）。WhisperX 资格语料自本轮起独立：`shared-corpus/self-made-faster-whisper-r3/manifest-whisperx-v2.json`（annotation 2、`manifest_sha256=5c6653eb…`，以 57.72 秒自造自然长音频 `natural-narration-zh` 替代人工 5 秒念编号片段；faster-whisper / qwen3 仍读共享 `manifest.json` v1、`manifest_sha256=cb2cad81…`），由仓库变量 `PRODUCTION_WHISPERX_QUALIFICATION_MANIFEST_PATH` 路由。本轮生产验证：活动 release 内只读重放 job `459010b5` 的退化窗口 `716.5-750s` / `779-802.5s`，由「248 字与 54 字编号/楼层平台复读」恢复为「12 段 108 字」「7 段 84 字」真实讲解；应用容器内 15 例术语诊断 `status=pass`。
- 方案链接：`docs/features/transcript-pipeline.md`
...
10. 生产转录可靠性修复：转录分块窗口 120s 固化进代码默认值、ASR 暂停任务重新入队、PyAV 音频解码、WhisperX 三轮修复（移除塌缩解码的 `repetition_penalty`、越界对齐片段裁剪而非整单永久失败、禁用 3-gram 重复消除热词复读）与 qualification 内容覆盖度门禁、ASR 提示词回吐发布门禁、转录版本清理与检索索引残留清理、恢复 D: 与 Actions 可用性的 ASR 存储回收（2026-09-12，PR #730/#770-#782，workflow `34652290722`/`34659496645`/`34663756860`/`34664390376`/`34665406479`/`34673676368`/`34674090875`/`34674989293`）。
