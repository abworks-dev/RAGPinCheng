...
- 下一步：
  ...
  - [ ] 完成 faster-whisper PASS diagnostic `runner_exit_code` 信息字段兼容修复的 scoped
    review、远端 CI 和合并；非零/空值不替代固定报告的 8 样本、5 gate 与三层确定性门禁。
...
1. faster-whisper 生产准入代码准备（2026-08-10）。
2. 管理端资料管理工作流 PR1，用户验收通过（2026-08-05）。
3. 多引擎视频自动转录架构决策与总体方案（2026-08-01）。
4. 查询拆分 Phase A 评测协议与指标实现（2026-07-31）。
5. 查询拆分 `retrieve_multi` rerank 超限修复及 79 题回归（2026-07-30）。
6. 黄金集索引指纹陈旧告警与严格模式（2026-07-30）。
7. 比较型黄金集、两侧覆盖评分和生产收益样本（2026-07-30）。
8. 黄金集第一期重建、生产新基线和用户验收（2026-07-30）。
9. 查询拆分核心、预算截断修复和生产冒烟（2026-07-30）。
10. Ubuntu 应用节点与 Windows GPU 节点迁移，稳定观察期结束（2026-07-30）。
