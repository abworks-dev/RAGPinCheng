...
- 下一步：
  ...
  - [ ] 基于已确认的 `jieba==0.42.1` 仅有 sdist、无法满足 `--only-binary=:all:`
    这一 blocker，在“构建受控内部 wheel、放宽 binary-only、调整环境隔离”中选择并审批；
    未批准前不得修改固定 pin、production freeze 或 Profile admission。
