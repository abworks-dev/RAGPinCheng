...
- 状态：待用户验收
- 目标：修复生产批量转录超时与转录稿残缺（含 WhisperX 解码塌缩只转录约四分之一语音）、制度上杜绝含 ASR 提示词回吐的旧稿被发布，并清理已删除转录版本的检索索引残留。
- 下一步：
  - [ ] 重新转录受影响的培训视频与 ASR 验收媒体，在工作台确认新稿包含开场白与完整讲解（修复前同一 120 秒窗口只有 6 段/72 字，修复后为 10 段/273 字）。
  - [ ] 抽查含回吐历史版本的视频，确认发布被拦截并提示重新转录生成新版本。
  - [ ] 为 WhisperX qualification 语料补一条 60 至 120 秒长音频样本并重跑认证：现语料仅 3.5 至 5.2 秒，实测短片段上 `repetition_penalty` 2.0 与 1.0 产出几乎一致（23 字比 21 字），无法覆盖长音频塌缩，`content_coverage` 门禁只能作为下限保护。
...
- 完成标准：批量转录不再超时且转录稿完整；WhisperX profile 出稿量与语速匹配；含回吐内容的版本无法发布；已删除版本的转录片段不再出现在检索结果中；用户确认验收通过。
- 依赖：ASR active release `34664390376`（whisperx 资格 run `34663756860`、契约 `3433b90bfbad916a1a7778fde2b6f660103438511019cb065225ccb01e89c6c7`；`ASR_CHUNK_DURATION_MS=120000` 与 `ASR_CHUNK_OVERLAP_MS=0` 已同时固化到 `services/asr_service/scheduler.py` 代码默认值）与应用 master tip `b789f2da`。
- 方案链接：`docs/features/transcript-pipeline.md`
...
10. 生产转录可靠性修复：转录分块窗口 120s 固化进代码默认值、ASR 暂停任务重新入队、PyAV 音频解码、WhisperX 解码塌缩修复（移除硬编码 `repetition_penalty`，同窗口出稿量 72 字升至 273 字）与 qualification 内容覆盖度门禁、ASR 提示词回吐发布门禁、转录版本清理与检索索引残留清理（2026-09-12，PR #730/#770/#771/#772/#773/#774/#775/#776/#777/#778，workflow `34652290722`/`34659496645`/`34663756860`/`34664390376`/`34665406479`）。
