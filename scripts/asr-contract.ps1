...
function Get-AsrRuntimeContract {
    ...
    return [ordered]@{ schema_version = "asr-runtime-contract/1"; engine = $Engine; source_commit_sha = $CommitSha.ToLowerInvariant(); runtime_contract_sha256 = Get-AsrContractSha256 -Text $canonical; manifest = $manifest }
}
...
function Get-AsrProductionAdmissionAdapter {
    ...
    switch ($Engine) {
        "faster-whisper" {
            return [ordered]@{ engine = $Engine; enabled = $true; evidence_adapter = "faster-whisper-r3/1" }
        }
        default {
            return [ordered]@{ engine = $Engine; enabled = $false; evidence_adapter = "" }
        }
    }
}
...
function Get-AsrDeploymentContract {
    ...
    return [ordered]@{ schema_version = "asr-deployment-contract/1"; source_commit_sha = $CommitSha.ToLowerInvariant(); deployment_contract_sha256 = Get-AsrContractSha256 -Text ($manifest | ConvertTo-Json -Depth 8 -Compress); manifest = $manifest }
}
