...
$expectedRepository = [IO.Path]::GetFullPath('D:\RAGPinCheng').TrimEnd('\')
$expectedRuntime = [IO.Path]::GetFullPath('D:\RAGPinCheng\runtime').TrimEnd('\')
...
if ($PSCmdlet.ShouldProcess($TaskName, "Register GPU runtime cleanup task in $(if ($EnableApply) { 'apply' } else { 'dry-run' }) mode")) {
    Register-ScheduledTask `
        ...
        -Principal $principal `
        -Settings $settings `
        -Description 'Dry-run or approved retention cleanup for D:\RAGPinCheng\runtime' `
        -Force | Out-Null
