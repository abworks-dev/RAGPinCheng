"""Build and validate the fixed internal jieba wheel for R3 qualification."""
...
PRELOADED_SDIST_PATH = Path(
    rf"${PRODUCTION_REPO_PATH}\runtime\qualification-inputs\faster-whisper\{SDIST_FILE_NAME}"
)
SETUPTOOLS_REQUIREMENT = "setuptools==80.9.0"
WHEEL_REQUIREMENT = "wheel==0.45.1"
SOURCE_DATE_EPOCH = "1579530443"
BUILD_REPETITIONS = 2
...
def validate_sdist(path: Path) -> None:
    if path.name != SDIST_FILE_NAME or not path.is_file():
        raise WheelContractError("Fixed jieba sdist is missing")
    if sha256_file(path) != SDIST_SHA256:
        raise WheelContractError("Fixed jieba sdist SHA-256 mismatch")
    if path.stat().st_size != SDIST_SIZE_BYTES:
        raise WheelContractError("Fixed jieba sdist size mismatch")
    ...
    with tarfile.open(path, mode="r:gz") as archive:
        ...
        if not members:
            raise WheelContractError("Fixed jieba sdist is empty")
        for member in members:
            relative = _safe_relative_path(member.name, label="sdist member")
            if member.issym() or member.islnk() or member.isdev() or member.isfifo():
                raise WheelContractError("Fixed jieba sdist contains a special member")
            if not (member.isfile() or member.isdir()):
                raise WheelContractError("Fixed jieba sdist contains an unknown member")
            if member.isfile():
                regular_files += 1
                if member.size < 0:
                    raise WheelContractError("Fixed jieba sdist contains an invalid size")
                if relative == PurePosixPath(_ROOT, "setup.py"):
                    saw_setup = member.size > 0
    if not saw_setup or regular_files == 0:
        raise WheelContractError("Fixed jieba sdist lacks its build entrypoint")
...
def _build_once(
    *,
    ...
    if len(wheels) != 1:
        raise WheelContractError("Fixed jieba build must produce exactly one wheel")
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        ...
        if len(set(wheel_hashes)) != 1:
            raise WheelContractError("Repeated jieba wheel builds are not reproducible")
        if built[0].name != built[1].name:
            raise WheelContractError("Repeated jieba wheel filenames differ")
