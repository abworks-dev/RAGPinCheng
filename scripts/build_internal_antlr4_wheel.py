"""Build and validate the fixed internal antlr4-python3-runtime wheel for R3 qualification."""
...
SDIST_FILE_NAME = "antlr4-python3-runtime-4.9.3.tar.gz"
SDIST_SHA256 = "f224469b4168294902bb1efa80a8bf7855f24c99aef99cbefc1bcd3cce77881b"
SDIST_SIZE_BYTES = 117034
SDIST_URL = (
...
def validate_sdist(path: Path) -> None:
    if path.name != SDIST_FILE_NAME or not path.is_file():
        raise WheelContractError("Fixed antlr4-python3-runtime sdist is missing")
    if sha256_file(path) != SDIST_SHA256:
        raise WheelContractError("Fixed antlr4-python3-runtime sdist SHA-256 mismatch")
    if path.stat().st_size != SDIST_SIZE_BYTES:
        raise WheelContractError("Fixed antlr4-python3-runtime sdist size mismatch")
    ...
    with tarfile.open(path, mode="r:gz") as archive:
        ...
        if not members:
            raise WheelContractError("Fixed antlr4-python3-runtime sdist is empty")
        for member in members:
            relative = _safe_relative_path(member.name, label="sdist member")
            if member.issym() or member.islnk() or member.isdev() or member.isfifo():
                raise WheelContractError("Fixed antlr4-python3-runtime sdist contains a special member")
            if not (member.isfile() or member.isdir()):
                raise WheelContractError("Fixed antlr4-python3-runtime sdist contains an unknown member")
            if member.isfile():
                regular_files += 1
                if member.size < 0:
                    raise WheelContractError("Fixed antlr4-python3-runtime sdist contains an invalid size")
                if relative == PurePosixPath(_ROOT, "setup.py"):
                    saw_setup = member.size > 0
    if not saw_setup or regular_files == 0:
        raise WheelContractError("Fixed antlr4-python3-runtime sdist lacks its build entrypoint")
...
def _download_fixed_sdist(destination: Path) -> None:
    ...
    with urllib.request.urlopen(request, timeout=60) as response:
        if response.geturl() != SDIST_URL:
            raise WheelContractError("Fixed antlr4-python3-runtime sdist download redirected")
        content_length = response.headers.get("Content-Length")
        if content_length is None or int(content_length) != SDIST_SIZE_BYTES:
            raise WheelContractError("Fixed antlr4-python3-runtime sdist response size mismatch")
        with destination.open("xb") as stream:
            remaining = SDIST_SIZE_BYTES
            while remaining:
                chunk = response.read(min(1024 * 1024, remaining))
                if not chunk:
                    raise WheelContractError("Fixed antlr4-python3-runtime sdist download was truncated")
                stream.write(chunk)
                remaining -= len(chunk)
            if response.read(1):
                raise WheelContractError("Fixed antlr4-python3-runtime sdist download exceeded fixed size")
...
def _build_once(
    *,
    ...
    if len(wheels) != 1:
        raise WheelContractError("Fixed antlr4-python3-runtime build must produce exactly one wheel")
...
def build_bundle(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="antlr4-python3-runtime-wheel-build-",
        ...
        if len(source_files) != 1 or source_files[0].name != SDIST_FILE_NAME:
            raise WheelContractError("PyPI did not return the fixed antlr4-python3-runtime sdist")
        ...
        if len(set(wheel_hashes)) != 1:
            raise WheelContractError("Repeated antlr4-python3-runtime wheel builds are not reproducible")
        if built[0].name != built[1].name:
            raise WheelContractError("Repeated antlr4-python3-runtime wheel filenames differ")
