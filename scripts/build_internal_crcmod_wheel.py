"""Build and validate the fixed internal crcmod wheel for R3 qualification."""
...
SDIST_FILE_NAME = "crcmod-1.7.tar.gz"
SDIST_SHA256 = "dc7051a0db5f2bd48665a990d3ec1cc305a466a77358ca4492826f41f283601e"
SDIST_SIZE_BYTES = 89670
SDIST_URL = (
...
def validate_sdist(path: Path) -> None:
    if path.name != SDIST_FILE_NAME or not path.is_file():
        raise WheelContractError("Fixed crcmod sdist is missing")
    if sha256_file(path) != SDIST_SHA256:
        raise WheelContractError("Fixed crcmod sdist SHA-256 mismatch")
    if path.stat().st_size != SDIST_SIZE_BYTES:
        raise WheelContractError("Fixed crcmod sdist size mismatch")
    ...
    with tarfile.open(path, mode="r:gz") as archive:
        ...
        if not members:
            raise WheelContractError("Fixed crcmod sdist is empty")
        for member in members:
            relative = _safe_relative_path(member.name, label="sdist member")
            if member.issym() or member.islnk() or member.isdev() or member.isfifo():
                raise WheelContractError("Fixed crcmod sdist contains a special member")
            if not (member.isfile() or member.isdir()):
                raise WheelContractError("Fixed crcmod sdist contains an unknown member")
            if member.isfile():
                regular_files += 1
                if member.size < 0:
                    raise WheelContractError("Fixed crcmod sdist contains an invalid size")
                if relative == PurePosixPath(_ROOT, "setup.py"):
                    saw_setup = member.size > 0
    if not saw_setup or regular_files == 0:
        raise WheelContractError("Fixed crcmod sdist lacks its build entrypoint")
...
def _download_fixed_sdist(destination: Path) -> None:
    ...
    with urllib.request.urlopen(request, timeout=60) as response:
        if response.geturl() != SDIST_URL:
            raise WheelContractError("Fixed crcmod sdist download redirected")
        content_length = response.headers.get("Content-Length")
        if content_length is None or int(content_length) != SDIST_SIZE_BYTES:
            raise WheelContractError("Fixed crcmod sdist response size mismatch")
        with destination.open("xb") as stream:
            remaining = SDIST_SIZE_BYTES
            while remaining:
                chunk = response.read(min(1024 * 1024, remaining))
                if not chunk:
                    raise WheelContractError("Fixed crcmod sdist download was truncated")
                stream.write(chunk)
                remaining -= len(chunk)
            if response.read(1):
                raise WheelContractError("Fixed crcmod sdist download exceeded fixed size")
...
def _build_once(
    *,
    ...
    if len(wheels) != 1:
        raise WheelContractError("Fixed crcmod build must produce exactly one wheel")
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        ...
        if len(source_files) != 1 or source_files[0].name != SDIST_FILE_NAME:
            raise WheelContractError("PyPI did not return the fixed crcmod sdist")
        ...
        if len(set(wheel_hashes)) != 1:
            raise WheelContractError("Repeated crcmod wheel builds are not reproducible")
        if built[0].name != built[1].name:
            raise WheelContractError("Repeated crcmod wheel filenames differ")
