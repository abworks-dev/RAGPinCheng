    ...
    foreach ($processId in @($listeners | ForEach-Object { $_.OwningProcess } | Sort-Object -Unique)) {
        ...
        if (
            ...
            (
                ...
                [string]$process.ExecutablePath -ne "C:\Program Files\Python310\python.exe"
            )
        ) {
...
function Invoke-SmokeTests {
    ...
    foreach ($attempt in 1..5) {
        $embedding = Invoke-RestMethod -Method Post -Uri "http://192.168.11.11:8100/v1/embeddings" `
        ...
        $rerank = Invoke-RestMethod -Method Post -Uri "http://192.168.11.11:8100/v1/rerank" `
            -Headers $headers -ContentType "application/json" -TimeoutSec 30 `
